...
echo "所属市场: $MARKET_NAME"
echo ""

# 方法 1: 使用 curl 从新浪财经获取数据
echo "🔍 正在查询股票信息..."
echo ""

SINA_URL="http://hq.sinajs.cn/list=$MARKET$STOCK_CODE"

# 获取数据
STOCK_DATA=$(curl -s "$SINA_URL" | iconv -f GBK -t UTF-8)
...
# 解析数据
STOCK_NAME=$(echo "$STOCK_DATA" | grep -oP '(?<=,)[^,]+(?=,)' | head -1)
CURRENT_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $3}')
YESTERDAY_CLOSE=$(echo "$STOCK_DATA" | awk -F',' '{print $2}')
OPEN_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $4}')
HIGH_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $5}')
LOW_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $6}')
BUY_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $7}')
SELL_PRICE=$(echo "$STOCK_DATA" | awk -F',' '{print $8}')
VOLUME=$(echo "$STOCK_DATA" | awk -F',' '{print $9}')
TURNOVER=$(echo "$STOCK_DATA" | awk -F',' '{print $10}')
...
if [ -n "$VOLUME" ] && [ "$VOLUME" != "0" ]; then
    VOLUME_HANDS=$(awk "BEGIN {printf \"%.2f\", $VOLUME / 100}")
