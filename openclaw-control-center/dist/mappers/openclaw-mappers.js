...
exports.mapSessionsListToSummaries = mapSessionsListToSummaries;
exports.mapCronListToSummaries = mapCronListToSummaries;
exports.mapApprovalsGetToSummaries = mapApprovalsGetToSummaries;
...
function toState(active, explicitState) {
    const normalized = explicitState?.trim().toLowerCase();
    if (normalized) {
        if (WAITING_APPROVAL_SESSION_STATES.has(normalized))
            return "waiting_approval";
        if (BLOCKED_SESSION_STATES.has(normalized))
            return "blocked";
        if (ERROR_SESSION_STATES.has(normalized))
            return "error";
        if (RUNNING_SESSION_STATES.has(normalized))
            return "running";
...
function toIso(updatedAtMs, updatedAt) {
    if (updatedAt)
        return updatedAt;
...
function mapSessionsListToSummaries(input) {
...
function mapCronListToSummaries(input) {
...
function mapApprovalsJsonToSummaries(json) {
    ...
    if (agents) {
        for (const [agentId, config] of Object.entries(agents)) {
            ...
            if (!cfg)
                continue;
            approvals.push(...mapApprovalArray(asArray(cfg.pending), "pending", `agent:${agentId}:pending`, agentId));
            approvals.push(...mapApprovalArray(asArray(cfg.requests), "pending", `agent:${agentId}:requests`, agentId));
            approvals.push(...mapApprovalArray(asArray(cfg.approvals), "unknown", `agent:${agentId}:approvals`, agentId));
            approvals.push(...mapApprovalArray(asArray(cfg.allowlist), "approved", `agent:${agentId}:allowlist`, agentId));
        }
    }
    return dedupeById(approvals);
}
function mapApprovalsTextToSummaries(rawText) {
...
function mapApprovalArray(array, fallbackState, prefix, agentIdFromContext) {
...
function mapApprovalRecord(record, fallbackState, fallbackId, agentIdFromContext) {
...
function extractFieldValueTable(text) {
    const result = {};
...
function toApprovalState(statusRaw, decisionRaw, fallback) {
    const value = `${statusRaw ?? ""} ${decisionRaw ?? ""}`.toLowerCase();
    if (value.includes("pending") || value.includes("wait"))
        return "pending";
    if (value.includes("approved") || value.includes("allow") || value.includes("granted"))
        return "approved";
    if (value.includes("denied") || value.includes("reject"))
        return "denied";
    return fallback;
}
function toInt(v) {
