...
exports.buildUsageCostSnapshot = buildUsageCostSnapshot;
exports.computeUsageCostSnapshot = computeUsageCostSnapshot;
exports.parseCodexRateLimitFromSessionLogForSmoke = parseCodexRateLimitFromSessionLogForSmoke;
...
const MODEL_CONTEXT_CATALOG_PATH = (0, node_path_1.join)(RUNTIME_DIR, "model-context-catalog.json");
const DEFAULT_SUBSCRIPTION_SNAPSHOT_PATH = (0, node_path_1.join)(RUNTIME_DIR, "subscription-snapshot.json");
const OPENCLAW_HOME = process.env.OPENCLAW_HOME?.trim() || (0, node_path_1.join)((0, node_os_1.homedir)(), ".openclaw");
const OPENCLAW_AGENTS_DIR = (0, node_path_1.join)(OPENCLAW_HOME, "agents");
const OPENCLAW_CRON_JOBS_PATH = (0, node_path_1.join)(OPENCLAW_HOME, "cron", "jobs.json");
const CODEX_HOME = process.env.CODEX_HOME?.trim() || (0, node_path_1.join)((0, node_os_1.homedir)(), ".codex");
const CODEX_SESSIONS_DIR = (0, node_path_1.join)(CODEX_HOME, "sessions");
const CODEX_RATE_LIMIT_CONNECTOR_PATH = (0, node_path_1.join)(CODEX_SESSIONS_DIR, "**", "*.jsonl");
...
const SUBSCRIPTION_SNAPSHOT_PATHS = [
    ...
    (0, node_path_1.join)(OPENCLAW_HOME, "subscription.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "subscription-snapshot.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "billing", "subscription.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "billing", "subscription-snapshot.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "billing", "usage.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "usage", "subscription.json"),
    (0, node_path_1.join)(OPENCLAW_HOME, "usage", "subscription-snapshot.json"),
].filter((item) => typeof item === "string" && item.trim().length > 0);
const RUNTIME_USAGE_EVENTS_CONNECTOR_PATH = (0, node_path_1.join)(OPENCLAW_AGENTS_DIR, "*", "sessions", "*.jsonl");
const RUNTIME_SESSION_INDEX_CONNECTOR_PATH = (0, node_path_1.join)(OPENCLAW_AGENTS_DIR, "*", "sessions", "sessions.json");
...
let usageDigestsCache;
let usageDigestsInFlight;
let modelContextCatalogCache;
let modelContextCatalogInFlight;
let runtimeUsageDataCache;
let runtimeUsageDataInFlight;
let openclawCronJobNameMapCache;
let openclawCronJobNameMapInFlight;
let subscriptionUsageWithCodexCache;
let subscriptionUsageWithCodexInFlight;
let subscriptionUsageWithoutCodexCache;
let subscriptionUsageWithoutCodexInFlight;
async function loadSourceWithCache(cache, inFlight, loader, assignCache, assignInFlight) {
    const now = Date.now();
...
async function loadCachedUsageDigests() {
    return loadSourceWithCache(usageDigestsCache, usageDigestsInFlight, loadUsageDigests, (value) => {
        usageDigestsCache = value;
    }, (value) => {
        usageDigestsInFlight = value;
    });
}
async function loadCachedModelContextCatalog() {
    return loadSourceWithCache(modelContextCatalogCache, modelContextCatalogInFlight, loadModelContextCatalog, (value) => {
        modelContextCatalogCache = value;
    }, (value) => {
        modelContextCatalogInFlight = value;
    });
}
async function loadCachedRuntimeUsageData() {
    return loadSourceWithCache(runtimeUsageDataCache, runtimeUsageDataInFlight, loadRuntimeUsageData, (value) => {
        runtimeUsageDataCache = value;
    }, (value) => {
        runtimeUsageDataInFlight = value;
    });
}
async function loadCachedOpenclawCronJobNameMap() {
    return loadSourceWithCache(openclawCronJobNameMapCache, openclawCronJobNameMapInFlight, loadOpenclawCronJobNameMap, (value) => {
...
async function loadCachedSubscriptionUsage(options = {}) {
    ...
    return loadSourceWithCache(includeCodexTelemetry ? subscriptionUsageWithCodexCache : subscriptionUsageWithoutCodexCache, includeCodexTelemetry ? subscriptionUsageWithCodexInFlight : subscriptionUsageWithoutCodexInFlight, () => loadSubscriptionUsage({ includeCodexTelemetry }), (value) => {
...
async function buildUsageCostSnapshot(snapshot, mode = "full") {
    if (mode === "summary") {
...
function computeUsageCostSnapshot(snapshot, digests, modelCatalog, runtimeUsage, subscriptionUsage, cronJobNameMap = new Map()) {
    ...
    const contextWindows = snapshot.statuses
        ...
        const usagePercent = contextLimitTokens && contextLimitTokens > 0 ? (usedTokens / contextLimitTokens) * 100 : undefined;
        ...
        if (runtimeSession?.contextWindowTokens && runtimeSession.contextWindowTokens > 0)
            runtimeContextRows += 1;
        return {
    ...
    const runtimeEvents30d = runtime.sourceStatus === "not_connected" ? [] : runtimeEventsWithinWindow(runtime.events, todayIso, 30);
    const runtimeEventsToday = runtime.sourceStatus === "not_connected" ? [] : runtimeEventsWithinWindow(runtime.events, todayIso, 1);
    const byAgent = runtime.sourceStatus === "not_connected"
    ...
    const byProject = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => {
            return sessionProjectMap.get(status.sessionKey) ?? "Unmapped project";
        })
        : aggregateBreakdownFromRuntime(runtimeEvents30d, runtime.sourceStatus, (event) => event.projectId ?? "Unmapped project");
    const byModel = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => status.model?.trim() || "Model not reported")
        : aggregateBreakdownFromRuntime(runtimeEvents30d, runtime.sourceStatus, (event) => event.model?.trim() || "Model not reported");
    const byTask = buildTaskBreakdownFromRuntime(snapshot, runtimeEvents30d, runtime.sourceStatus);
    const byProvider = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => inferProvider(status.model))
        : aggregateBreakdownFromRuntime(runtimeEvents30d, runtime.sourceStatus, (event) => event.provider?.trim() || inferProvider(event.model));
    const bySessionType = buildSessionTypeBreakdownFromSessionContexts(runtimeUsage?.sessionContexts ?? [], runtime.sourceStatus);
    const byCronJob = buildCronJobBreakdownFromSessionContexts(runtimeUsage?.sessionContexts ?? [], runtime.sourceStatus, cronJobNameMap);
    const byCronAgent = buildCronAgentBreakdownFromSessionContexts(runtimeUsage?.sessionContexts ?? [], runtime.sourceStatus);
    const byAgentToday = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => {
            const session = sessionByKey.get(status.sessionKey);
            return session?.agentId ?? "Unassigned";
        })
        : aggregateBreakdownFromRuntime(runtimeEventsToday, runtime.sourceStatus, (event) => event.agentId || "Unassigned");
    const byProjectToday = runtime.sourceStatus === "not_connected"
    ...
    const byModelToday = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => status.model?.trim() || "Model not reported")
    ...
    const byProviderToday = runtime.sourceStatus === "not_connected"
        ? aggregateBreakdownFromStatuses(snapshot.statuses, (status) => inferProvider(status.model))
        : aggregateBreakdownFromRuntime(runtimeEventsToday, runtime.sourceStatus, (event) => event.provider?.trim() || inferProvider(event.model));
...
async function loadUsageDigests() {
    let files = [];
    try {
    ...
    for (const fileName of jsonFiles) {
        try {
            const raw = JSON.parse(await (0, promises_1.readFile)((0, node_path_1.join)(DIGEST_DIR, fileName), "utf8"));
...
async function loadModelContextCatalog() {
    try {
        const raw = JSON.parse(await (0, promises_1.readFile)(MODEL_CONTEXT_CATALOG_PATH, "utf8"));
        ...
        for (const entry of models) {
            ...
            if (!match || contextWindowTokens === undefined)
                continue;
            entries.push({
...
async function loadOpenclawCronJobNameMap() {
    const out = new Map();
    try {
        const raw = JSON.parse(await (0, promises_1.readFile)(OPENCLAW_CRON_JOBS_PATH, "utf8"));
...
async function loadRuntimeUsageData() {
    const out = {
    ...
    let agentDirs = [];
    try {
        const entries = await (0, promises_1.readdir)(OPENCLAW_AGENTS_DIR, { withFileTypes: true });
        agentDirs = entries
            .filter((entry) => entry.isDirectory())
            .map((entry) => ({
            name: entry.name,
            path: (0, node_path_1.join)(OPENCLAW_AGENTS_DIR, entry.name),
        }));
    }
    catch {
        return out;
    ...
    for (const agent of agentDirs) {
        const sessionsDir = (0, node_path_1.join)(agent.path, "sessions");
        ...
        for (const context of contextResult.contexts) {
            ...
            if (context.sessionId)
                sessionById.set(context.sessionId, context);
        }
        const eventResult = await loadRuntimeUsageEventsForAgent(agent.name, sessionsDir, lookbackLowerBoundMs, sessionById);
...
async function loadRuntimeSessionContextsForAgent(agentId, sessionsDir) {
    const contexts = [];
    const sessionsIndexPath = (0, node_path_1.join)(sessionsDir, "sessions.json");
    try {
        const raw = JSON.parse(await (0, promises_1.readFile)(sessionsIndexPath, "utf8"));
        for (const [sessionKey, value] of Object.entries(raw)) {
            const obj = asObject(value);
            if (!obj || !sessionKey.trim())
                continue;
...
async function loadRuntimeUsageEventsForAgent(agentId, sessionsDir, lookbackLowerBoundMs, sessionById) {
    const events = [];
    let fileNames = [];
    try {
        fileNames = await (0, promises_1.readdir)(sessionsDir);
    }
    catch (error) {
        if (isFsNotFound(error)) {
            return {
    ...
    for (const fileName of sessionFiles) {
        const filePath = (0, node_path_1.join)(sessionsDir, fileName);
        try {
            const fileStat = await (0, promises_1.stat)(filePath);
            if (fileStat.mtimeMs < lookbackLowerBoundMs)
                continue;
            ...
            for (const line of lines) {
                ...
                if (asString(parsed.type) !== "message")
                    continue;
                ...
                if (!Number.isFinite(timestampMs) || timestampMs < lookbackLowerBoundMs)
                    continue;
...
function resolveRuntimeUsage(runtime, sessionProjectMap) {
    if (!runtime) {
        return {
            sourceStatus: "not_connected",
    ...
    for (const item of runtime.sessionContexts) {
        if (!item.sessionKey.trim())
            continue;
        sessionByKey.set(item.sessionKey, item);
        if (item.sessionId)
            sessionById.set(item.sessionId, item);
    }
...
function buildUsagePeriods(snapshot, digests, todayIso, runtime) {
    const windows = [
    ...
    return windows.map((window) => {
        ...
        const aggregate = within.reduce((acc, item) => {
        ...
        const currentDailyAverage = aggregate.cost / Math.max(1, window.days);
        const sourceStatus = within.length > 0
            ? "connected"
...
function runtimeEventsWithinWindow(events, todayIso, days) {
...
function aggregateRuntimeUsageWindow(events) {
    const days = new Set();
    let tokens = 0;
    let cost = 0;
...
function buildRuntimeDailyCostMap(events) {
    const byDay = new Map();
    for (const event of events) {
...
function previousWindowAverageCostFromDailyMap(dailyCostByDay, todayIso, days) {
    const todayMs = toDayMs(todayIso);
    if (!Number.isFinite(todayMs))
        return undefined;
...
function digestsWithinWindow(digests, todayIso, days) {
    const todayMs = toDayMs(todayIso);
    if (!Number.isFinite(todayMs))
        return [];
    const lowerBound = todayMs - (days - 1) * DAY_MS;
...
function previousWindowAverageCost(digests, todayIso, days) {
    const todayMs = toDayMs(todayIso);
    if (!Number.isFinite(todayMs))
        return undefined;
    const currentLower = todayMs - (days - 1) * DAY_MS;
...
function classifyPace(currentDailyAverage, baselineDailyAverage, baselineSourceAvailable) {
...
function buildSessionProjectMap(snapshot) {
    const out = new Map();
    for (const task of snapshot.tasks.tasks) {
        for (const sessionKey of task.sessionKeys) {
            if (!out.has(sessionKey))
                out.set(sessionKey, task.projectId);
        }
    }
    return out;
}
function aggregateBreakdownFromStatuses(statuses, keySelector) {
    const byKey = new Map();
...
function aggregateBreakdownFromRuntime(events, sourceStatus, keySelector) {
...
function buildSessionTypeBreakdownFromSessionContexts(contexts, sourceStatus) {
    if (sourceStatus === "not_connected" || contexts.length === 0)
        return [];
    const uniqueSessions = dedupeSessionContexts(contexts);
    if (uniqueSessions.length === 0)
        return [];
    const order = ["Cron", "Discord", "Telegram", "Main/内部会话"];
    const buckets = new Map(order.map((label) => [
        label,
        {
    ...
    for (const session of uniqueSessions) {
        if (!Number.isFinite(session.totalTokens) || session.totalTokens <= 0)
            continue;
    ...
    return order
        ...
        .filter((row) => Boolean(row) && (row?.tokens ?? 0) > 0);
}
function buildSessionTypeBreakdownFromRuntimeEvents(events, sourceStatus) {
    if (sourceStatus === "not_connected" || events.length === 0)
        return [];
    const order = ["Cron", "Discord", "Telegram", "Main/内部会话"];
    const buckets = new Map(order.map((label) => [
        label,
    ...
    const seenSessionsByLabel = new Map();
    for (const event of events) {
        ...
        bucket.requests += 1;
        const seen = seenSessionsByLabel.get(label) ?? new Set();
...
function buildCronJobBreakdownFromSessionContexts(contexts, sourceStatus, cronJobNameMap) {
    if (sourceStatus === "not_connected" || contexts.length === 0)
        return [];
    const uniqueSessions = dedupeSessionContexts(contexts);
    if (uniqueSessions.length === 0)
    ...
    for (const session of uniqueSessions) {
        if (!Number.isFinite(session.totalTokens) || session.totalTokens <= 0)
            continue;
        if (classifySessionTypeLabel(session) !== "Cron")
            continue;
...
function buildCronJobBreakdownFromRuntimeEvents(events, sourceStatus, cronJobNameMap) {
    if (sourceStatus === "not_connected" || events.length === 0)
        return [];
    const buckets = new Map();
    const seenSessionsByKey = new Map();
    for (const event of events) {
        if (classifySessionTypeFromSessionKey(event.sessionKey) !== "Cron")
            continue;
        ...
        bucket.estimatedCost += event.cost;
        bucket.requests += 1;
        const seen = seenSessionsByKey.get(key) ?? new Set();
        seen.add(event.sessionKey ?? event.sessionId);
...
function buildCronAgentBreakdownFromSessionContexts(contexts, sourceStatus) {
    if (sourceStatus === "not_connected" || contexts.length === 0)
        return [];
    const uniqueSessions = dedupeSessionContexts(contexts);
    if (uniqueSessions.length === 0)
    ...
    for (const session of uniqueSessions) {
        ...
        const current = buckets.get(agentId) ??
            {
                key: agentId,
                label: agentId,
                tokens: 0,
...
function buildCronAgentBreakdownFromRuntimeEvents(events, sourceStatus) {
    if (sourceStatus === "not_connected" || events.length === 0)
        return [];
    const buckets = new Map();
    const seenSessionsByAgent = new Map();
    for (const event of events) {
        if (classifySessionTypeFromSessionKey(event.sessionKey) !== "Cron")
            continue;
        ...
        bucket.estimatedCost += event.cost;
        bucket.requests += 1;
        const seen = seenSessionsByAgent.get(agentId) ?? new Set();
        seen.add(event.sessionKey ?? event.sessionId);
...
function dedupeSessionContexts(contexts) {
...
function classifySessionTypeLabel(context) {
...
function classifySessionTypeFromSessionKey(sessionKey) {
    const key = sessionKey?.trim().toLowerCase() ?? "";
    if (!key)
        return "Main/内部会话";
    if (key.includes(":cron:") || key.startsWith("cron:"))
        return "Cron";
    if (key.includes(":discord:") || key.startsWith("discord:"))
        return "Discord";
    if (key.includes(":telegram:") || key.startsWith("telegram:"))
        return "Telegram";
    return "Main/内部会话";
}
function parseCronJobIdFromSessionKey(sessionKey) {
...
function buildTaskBreakdownFromRuntime(snapshot, events, sourceStatus) {
    if (sourceStatus === "not_connected" || events.length === 0)
        return [];
    ...
    const sessionKeyToTaskKeys = new Map();
    const ownerToActiveTaskKeys = new Map();
    ...
    const buckets = new Map();
    const assignEventToTask = (taskKey, event, weight, methodLabel) => {
        const meta = taskKeyToMeta.get(taskKey);
        if (!meta)
            return;
        const bucket = buckets.get(taskKey) ??
            {
                row: {
                    key: taskKey,
    ...
    for (const event of events) {
        ...
        if (fromSession && fromSession.length > 0) {
            ...
            for (const taskKey of fromSession)
                assignEventToTask(taskKey, event, weight, "session");
            continue;
        }
        ...
        if (ownerTasks && ownerTasks.length > 0) {
            ...
            for (const taskKey of ownerTasks)
                assignEventToTask(taskKey, event, weight, "owner");
            continue;
        }
        if (activeTaskKeys.length > 0) {
            const weight = 1 / activeTaskKeys.length;
            for (const taskKey of activeTaskKeys)
                assignEventToTask(taskKey, event, weight, "estimate");
            continue;
        }
        const bucket = buckets.get(unmappedKey) ??
            {
                row: {
                    key: unmappedKey,
...
function resolveContextCatalogEntry(catalog, model) {
    const normalized = model.trim().toLowerCase();
    if (!normalized)
        return undefined;
    return catalog.find((entry) => normalized.includes(entry.match));
}
function resolveContextThresholdState(usagePercent) {
    if (usagePercent === undefined)
        return "not_connected";
    if (usagePercent >= CONTEXT_CRITICAL_RATIO * 100)
        return "critical";
    if (usagePercent >= CONTEXT_WARN_RATIO * 100)
        return "warn";
    return "ok";
}
function resolveContextPaceLabel(usagePercent, updatedAt) {
    if (usagePercent === undefined)
        return "Data source not connected";
    const ageMinutes = ageInMinutes(updatedAt);
    if (usagePercent >= CONTEXT_CRITICAL_RATIO * 100)
        return ageMinutes <= 20 ? "Fast burn" : "High load";
    if (usagePercent >= CONTEXT_WARN_RATIO * 100)
        return ageMinutes <= 20 ? "Rising" : "Watch closely";
    return ageMinutes <= 20 ? "Steady" : "Cooling";
}
function formatContextThresholds(contextLimitTokens) {
    ...
    return `Warn ${formatNumber(warnTokens)} (${Math.round(CONTEXT_WARN_RATIO * 100)}%), critical ${formatNumber(criticalTokens)} (${Math.round(CONTEXT_CRITICAL_RATIO * 100)}%)`;
}
function buildUsageBudgetStatus(snapshot, period30) {
    ...
    const burnRatePerDay = period30 && period30.sourceStatus !== "not_connected"
        ? usedCost30d / Math.max(1, period30.daysCovered || 1)
        : undefined;
    const projectedDaysToLimit = burnRatePerDay && burnRatePerDay > 0 ? (limitCost30d - usedCost30d) / burnRatePerDay : undefined;
    if (ratio >= 1) {
        return {
            status: "over",
            usedCost30d,
...
function buildConnectorStatus(input) {
    ...
    if (!input.hasContextCatalog && !input.hasRuntimeContext) {
        todos.push({
            ...
            detail: "Create runtime/model-context-catalog.json with {match, contextWindowTokens, provider} entries to enable context percentages.",
        });
    }
    ...
    if (!input.hasRequestCounts) {
        todos.push({
            ...
            detail: "Enable OpenClaw session runtime stores so Usage & Cost can read real request counts from assistant usage logs.",
        });
    }
...
function subscriptionTodoTitle(reasonCode) {
    if (reasonCode === "provider_snapshot_unreadable")
        return "Fix unreadable subscription snapshot";
    if (reasonCode === "runtime_backfill_only")
        return "Connect provider subscription snapshot";
    if (reasonCode === "provider_snapshot_missing")
        return "Connect subscription usage snapshot";
    return "Complete subscription usage fields";
}
function subscriptionTodoDetail(reasonCode, detail, connectHint) {
...
async function loadSubscriptionUsage(options = {}) {
    const includeCodexTelemetry = options.includeCodexTelemetry !== false;
    ...
    for (const path of SUBSCRIPTION_SNAPSHOT_PATHS) {
        try {
            const raw = JSON.parse(await (0, promises_1.readFile)(path, "utf8"));
    ...
    if (codexRateLimitUsage?.status === "connected")
        return codexRateLimitUsage;
    if (partial)
        return partial;
    if (codexRateLimitUsage)
        return codexRateLimitUsage;
    if (unreadablePaths.length > 0) {
        const missingSegment = missingPaths.length > 0 ? ` Missing path(s): ${missingPaths.join(", ")}.` : "";
        return {
            status: "partial",
...
function finalizeSubscriptionUsage(subscriptionUsage, period30, budget) {
    const runtimeConsumedAvailable = period30 !== undefined &&
    ...
    const budgetLimit = budget && typeof budget.limitCost30d === "number" && Number.isFinite(budget.limitCost30d) && budget.limitCost30d > 0
        ? budget.limitCost30d
        : undefined;
    ...
    if (!subscriptionUsage || subscriptionUsage.status === "not_connected") {
        if (runtimeConsumed === undefined)
            return subscriptionUsage ?? defaultSubscriptionUsage();
        if (budgetLimit !== undefined) {
            ...
            return {
                ...
                cycleEnd: todayIso,
                sourcePath: `${RUNTIME_USAGE_EVENTS_CONNECTOR_PATH} (usage events); ` +
                    `${RUNTIME_SESSION_INDEX_CONNECTOR_PATH} (session index); ` +
                    `${SUBSCRIPTION_BUDGET_FALLBACK_SOURCE}`,
                detail: `Consumed is derived from runtime usage (${period30?.label ?? "Last 30 days"}). ` +
    ...
    const remaining = subscriptionUsage.remaining ??
    ...
    const providerConnected = subscriptionUsage.reasonCode === "provider_connected" &&
    ...
    const strictRemaining = subscriptionUsage.remaining ??
        (typeof consumed === "number" && typeof strictLimit === "number" ? Math.max(0, strictLimit - consumed) : undefined);
    const usagePercent = typeof consumed === "number" && typeof strictLimit === "number" && strictLimit > 0
    ...
    const detail = [
        ...
        usedBudgetFallbackForLimit
            ? `Limit/remaining are estimated from configured 30d cost budget in snapshot budgetSummary (${formatNumber(budgetLimit ?? 0)} ${subscriptionUsage.unit}).`
            : "",
        ...
        .join(" ");
    const reasonCode = subscriptionUsage.reasonCode === "provider_snapshot_unreadable"
...
async function loadCodexRateLimitUsage(connectHint) {
    let fileEntries = [];
    try {
    ...
    for (const entry of fileEntries) {
        let parsed;
        try {
            parsed = parseCodexRateLimitFromSessionLog(await (0, promises_1.readFile)(entry.path, "utf8"), entry.path, entry.mtimeMs);
        }
        catch {
            continue;
        }
        if (!parsed)
            continue;
        if (!latest || compareCodexRateLimitSnapshots(parsed, latest) > 0)
            latest = parsed;
    }
    if (!latest)
        return undefined;
    ...
    const cycleStart = cycleEnd && latest.primaryWindowMinutes
        ? new Date(Date.parse(cycleEnd) - latest.primaryWindowMinutes * 60 * 1000).toISOString()
        : undefined;
    const secondaryUsageLabel = latest.secondaryUsedPercent !== undefined
    ...
    return {
        ...
        detail: `来自 Codex CLI 实时额度信号（limit=${latest.limitId ?? "unknown"}，plan=${planType}，主窗口 ${primaryWindowLabel || "unknown"}）。` +
        ...
        secondaryRemainingPercent: latest.secondaryUsedPercent !== undefined ? Math.max(0, 100 - latest.secondaryUsedPercent) : undefined,
        secondaryResetAt: toIsoFromEpoch(latest.secondaryResetAtMs),
        connectHint,
        reasonCode: "provider_connected",
    };
}
function parseCodexRateLimitFromSessionLog(raw, sourcePath, fallbackTimestampMs) {
    const lines = raw.replace(/\r/g, "").split("\n");
    let latest;
    for (const line of lines) {
        const trimmed = line.trim();
        ...
        if (asString(parsed.type) !== "event_msg")
            continue;
        const payload = asObject(parsed.payload);
        if (!payload || asString(payload.type) !== "token_count")
            continue;
        ...
        if (!latest || compareCodexRateLimitSnapshots(snapshot, latest) > 0)
            latest = snapshot;
    }
    return latest;
}
function compareCodexRateLimitSnapshots(a, b) {
...
function codexRateLimitPriority(snapshot) {
    const limitId = snapshot.limitId?.trim().toLowerCase() ?? "";
    const limitName = snapshot.limitName?.trim().toLowerCase() ?? "";
    if (limitId === "codex")
        return 300;
    if (limitId.startsWith("codex_"))
        return 200;
    if (limitName.includes("codex"))
        return 150;
    return 100;
}
function parseCodexRateLimitFromSessionLogForSmoke(raw, sourcePath = "/tmp/codex-session.jsonl", fallbackTimestampMs = 0) {
    ...
    return {
        ...
        secondaryWindowLabel: formatWindowMinutesLabel(snapshot.secondaryWindowMinutes) ||
            (snapshot.secondaryUsedPercent !== undefined ? "Week" : undefined),
        secondaryUsedPercent: snapshot.secondaryUsedPercent,
        secondaryRemainingPercent: snapshot.secondaryUsedPercent !== undefined ? Math.max(0, 100 - snapshot.secondaryUsedPercent) : undefined,
        secondaryResetAt: toIsoFromEpoch(snapshot.secondaryResetAtMs),
    };
}
async function collectRecentJsonlFiles(rootDir, limit) {
    const dirs = [rootDir];
    const files = [];
    while (dirs.length > 0) {
        ...
        for (const entry of entries) {
            const entryPath = (0, node_path_1.join)(current, entry.name);
...
function subscriptionConnectHint() {
...
function parseEpochMaybeSeconds(input) {
...
function parseSubscriptionUsage(input, sourcePath, connectHint) {
    ...
    const billing = asObject(root.billingCycle) ??
        asObject(root.cycle) ??
    ...
    const sourceLabel = (asString(meta.source) ??
        asString(root.source) ??
        asString(root.sourceType) ??
        "").toLowerCase();
    const isSyntheticSnapshot = sourceLabel.includes("runtime_backfill") ||
        sourceLabel.includes("estimated") ||
        sourceLabel.includes("bootstrap");
    const planLabel = asString(planObj.name) ??
        ...
        asString(root.tier) ??
        "Subscription";
    const unit = asString(subscription.unit) ??
        asString(root.unit) ??
        asString(usage.unit) ??
    ...
    const cycleStart = asString(subscription.cycleStart) ??
    ...
    const cycleEnd = asString(subscription.cycleEnd) ??
...
function defaultSubscriptionUsage(missingPaths = SUBSCRIPTION_SNAPSHOT_PATHS) {
    const missingDetail = missingPaths.length > 0
...
function contextThresholdRank(state) {
    if (state === "critical")
        return 0;
    if (state === "warn")
        return 1;
...
function asNonNegativeNumber(input) {
    if (typeof input !== "number" || !Number.isFinite(input) || input < 0)
        return undefined;
    return input;
}
