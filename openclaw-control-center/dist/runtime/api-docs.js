...
function buildApiDocs() {
    return {
        ...
        safetyDefaults: {
            ...
            LOCAL_API_TOKEN: "Set explicitly to allow import/export and state-changing routes; present token via x-local-token or Authorization Bearer",
            approvalExecutionGuard: "Live approve/reject requires READONLY_MODE=false + APPROVAL_ACTIONS_ENABLED=true + APPROVAL_ACTIONS_DRY_RUN=false",
            importMutationExecutionGuard: "Live import apply requires LOCAL_API_TOKEN auth + IMPORT_MUTATION_ENABLED=true + READONLY_MODE=false; optional per-request dryRun=true keeps it non-mutating",
            taskHeartbeatExecutionGuard: "Live task heartbeat execution requires LOCAL_API_TOKEN when LOCAL_TOKEN_AUTH_REQUIRED=true; default mode is dry-run",
        },
        routes: [
            {
                method: "GET",
            ...
            },
            {
                method: "PATCH",
                path: "/api/ui/preferences",
                summary: "Update dashboard UI preferences persisted to runtime/ui-preferences.json (requires local token gate)",
                body: {
            ...
            },
            {
                method: "GET",
                path: "/api/usage-cost",
                summary: "Usage/cost observability snapshot with context-window, period totals, burn-rate status, and connector TODOs",
                response: {
                    ok: "boolean",
                    usage: "{ periods(today/7d/30d), contextWindows[], breakdown(byAgent/byProject/byModel/byProvider), budget, connectors }",
                },
            },
            {
                method: "GET",
                ...
                response: {
                    ok: "boolean",
                    replay: "{ timeline, digests, exportSnapshots, exportBundles, stats:{timeline,digests,exportSnapshots,exportBundles,total} with per-source latencyMs/latencyBucketsMs(p50,p95)/totalSizeBytes/returnedSizeBytes }",
                },
            },
            {
                method: "GET",
            ...
            },
            {
                ...
                summary: "Preview stale acknowledgement prune counts (no write, requires local token gate)",
                response: {
                    ok: "boolean",
            ...
            },
            {
                method: "POST",
                path: "/api/tasks/heartbeat",
                summary: "Execute heartbeat task pickup (requires local token gate; defaults to dry-run unless explicitly set live)",
                body: {
            ...
            },
            {
                ...
                summary: "Export state bundle and persist timestamped debug + backup snapshots (requires local token gate)",
                response: {
                    ok: "boolean",
            ...
            },
            {
                ...
                summary: "Validate exported bundle shape in dry-run mode (no state mutation, requires local token gate)",
                body: {
            ...
            },
            {
                ...
                summary: "Optional local import mutation endpoint (HIGH RISK): requires local token + IMPORT_MUTATION_ENABLED=true; blocked in readonly unless dryRun=true",
                body: {
                    fileName: "optional runtime/exports/*.json name or path",
