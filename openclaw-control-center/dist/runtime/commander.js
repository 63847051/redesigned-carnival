...
exports.commanderAlerts = commanderAlerts;
exports.commanderExceptions = commanderExceptions;
exports.commanderExceptionsFeed = commanderExceptionsFeed;
const task_store_1 = require("./task-store");
const CURRENT_RUNTIME_ISSUE_WINDOW_MS = 6 * 60 * 60 * 1000;
function commanderAlerts(snapshot) {
    const alerts = [];
...
function commanderExceptions(snapshot) {
    const nowMs = toMs(snapshot.generatedAt) || Date.now();
    const blocked = snapshot.sessions.filter((s) => (s.state === "blocked" || s.state === "waiting_approval") &&
        isFreshRuntimeIssueSession(s.lastMessageAt, nowMs));
    const errors = snapshot.sessions.filter((s) => s.state === "error" && isFreshRuntimeIssueSession(s.lastMessageAt, nowMs));
    ...
    const tasksDue = (0, task_store_1.listTasks)(snapshot.tasks, projectTitleById).filter((task) => {
        if (!task.dueAt)
            return false;
        if (task.status === "done")
            return false;
...
function commanderExceptionsFeed(snapshot) {
    const items = [];
    const exceptions = commanderExceptions(snapshot);
...
function isFreshRuntimeIssueSession(lastMessageAt, nowMs) {
    const latestAt = toMs(lastMessageAt);
    if (latestAt <= 0)
        return true;
    return nowMs - latestAt <= CURRENT_RUNTIME_ISSUE_WINDOW_MS;
}
function deriveBudgetOccurredAt(snapshot, budget) {
