...
exports.resolveDashboardSection = resolveDashboardSection;
exports.resolveLegacyDashboardSectionForSmoke = resolveLegacyDashboardSectionForSmoke;
exports.buildDashboardSearchResultForSmoke = buildDashboardSearchResultForSmoke;
exports.resolveEditableAgentScopesFromConfigForSmoke = resolveEditableAgentScopesFromConfigForSmoke;
exports.resolveEditableAgentScopesWithFallbackForSmoke = resolveEditableAgentScopesWithFallbackForSmoke;
exports.deriveAgentAnimalIdentity = deriveAgentAnimalIdentity;
exports.buildOfficeAgentRosterIds = buildOfficeAgentRosterIds;
exports.buildOfficeSpaceCards = buildOfficeSpaceCards;
exports.buildExecutionAgentSummaries = buildExecutionAgentSummaries;
exports.buildStaffOverviewCards = buildStaffOverviewCards;
exports.renderSubscriptionStatusCardForSmoke = renderSubscriptionStatusCardForSmoke;
exports.renderDashboardSectionNavForSmoke = renderDashboardSectionNavForSmoke;
exports.renderGlobalVisibilityCardForSmoke = renderGlobalVisibilityCardForSmoke;
exports.renderInformationCertaintyCardForSmoke = renderInformationCertaintyCardForSmoke;
exports.renderTaskCertaintySectionForSmoke = renderTaskCertaintySectionForSmoke;
exports.renderTaskExecutionChainCardsForSmoke = renderTaskExecutionChainCardsForSmoke;
exports.renderSessionDrilldownPageForSmoke = renderSessionDrilldownPageForSmoke;
exports.renderAuditPageForSmoke = renderAuditPageForSmoke;
...
const api_docs_1 = require("../runtime/api-docs");
const budget_governance_1 = require("../runtime/budget-governance");
const budget_policy_1 = require("../runtime/budget-policy");
const commander_1 = require("../runtime/commander");
const cron_overview_1 = require("../runtime/cron-overview");
const done_checklist_1 = require("../runtime/done-checklist");
const audit_timeline_1 = require("../runtime/audit-timeline");
const digest_renderer_1 = require("../runtime/digest-renderer");
const export_bundle_1 = require("../runtime/export-bundle");
const healthz_1 = require("../runtime/healthz");
const import_live_1 = require("../runtime/import-live");
const import_dry_run_1 = require("../runtime/import-dry-run");
const local_token_auth_1 = require("../runtime/local-token-auth");
const operation_audit_1 = require("../runtime/operation-audit");
const approval_action_service_1 = require("../runtime/approval-action-service");
const action_queue_links_1 = require("../runtime/action-queue-links");
const replay_index_1 = require("../runtime/replay-index");
const notification_policy_1 = require("../runtime/notification-policy");
const notification_center_1 = require("../runtime/notification-center");
const pixel_state_1 = require("../runtime/pixel-state");
const usage_cost_1 = require("../runtime/usage-cost");
...
const ui_preferences_1 = require("../runtime/ui-preferences");
const task_store_1 = require("../runtime/task-store");
const session_conversations_1 = require("../runtime/session-conversations");
const agent_roster_1 = require("../runtime/agent-roster");
const office_session_presence_1 = require("../runtime/office-session-presence");
const SNAPSHOT_PATH = (0, node_path_1.join)(process.cwd(), "runtime", "last-snapshot.json");
const OPENCLAW_HOME_DIR = process.env.OPENCLAW_HOME?.trim() || (0, node_path_1.join)((0, node_os_1.homedir)(), ".openclaw");
const OPENCLAW_CRON_JOBS_CANDIDATES = [
    (0, node_path_1.join)(OPENCLAW_HOME_DIR, "cron", "jobs.json"),
    (0, node_path_1.join)(process.cwd(), "..", "..", "..", "..", "cron", "jobs.json"),
];
const DOCS_DIR = (0, node_path_1.join)(process.cwd(), "docs");
const README_PATH = (0, node_path_1.join)(process.cwd(), "README.md");
const AGENT_ROOT_DIR = (0, node_path_1.join)(process.cwd(), "..");
const MEMORY_DIR_CANDIDATES = [
    (0, node_path_1.join)(AGENT_ROOT_DIR, "memory"),
    (0, node_path_1.join)(process.cwd(), "runtime", "digests"),
];
const LONG_TERM_MEMORY_FILE_CANDIDATES = [
    (0, node_path_1.join)(AGENT_ROOT_DIR, "MEMORY.md"),
    (0, node_path_1.join)(AGENT_ROOT_DIR, "USER.md"),
    (0, node_path_1.join)(AGENT_ROOT_DIR, "SOUL.md"),
    (0, node_path_1.join)(AGENT_ROOT_DIR, "IDENTITY.md"),
    (0, node_path_1.join)(OPENCLAW_HOME_DIR, "memory", "MEMORY.md"),
];
const DOC_HUB_DIR_CANDIDATES = [
    { dir: DOCS_DIR, category: "项目文档" },
    { dir: (0, node_path_1.join)(process.cwd(), "runtime", "digests"), category: "日报文档" },
    { dir: (0, node_path_1.join)(process.cwd(), "runtime", "evidence"), category: "证据报告" },
];
const DOC_HUB_CHAT_INDEX_PATH = (0, node_path_1.join)(process.cwd(), "runtime", "doc-hub-chat.json");
const HTML_HEAVY_CACHE_TTL_MS = 3_000;
const HTML_USAGE_CACHE_TTL_MS = 10_000;
const HTML_SNAPSHOT_CACHE_TTL_MS = 10_000;
const HTML_LIVE_SESSIONS_CACHE_TTL_MS = config_1.POLLING_INTERVALS_MS.sessionsList;
...
const OPENCLAW_WORKSPACE_ROOT = (0, node_path_1.resolve)(process.cwd(), "..", "..", "..");
...
const SHARED_DOCUMENT_FILE_CANDIDATES = [
    ...
    "HEARTBEAT.md",
    "TOOLS.md",
    (0, node_path_1.join)(".learnings", "LEARNINGS.md"),
    (0, node_path_1.join)("control-center", "README.md"),
];
...
const OPENCLAW_CONFIG_PATH = (0, node_path_1.join)(OPENCLAW_HOME_DIR, "openclaw.json");
...
function startUiServer(port, toolClient) {
    const approvalActions = new approval_action_service_1.ApprovalActionService(toolClient);
    const server = (0, node_http_1.createServer)(async (req, res) => {
        ...
        try {
            ...
            if (method === "GET" && (path === "/" || legacySection)) {
                const prefs = await (0, ui_preferences_1.loadUiPreferences)();
                ...
                if (hasAnyQueryKey(url.searchParams, ["quick", "status", "owner", "project", "compact", "lang", "usage_view"])) {
                    await (0, ui_preferences_1.saveUiPreferences)({
                        language,
                        compactStatusStrip,
            ...
            if (method === "GET" && path.startsWith("/docs/")) {
                ...
                let body = "";
                try {
                    body = await (0, promises_1.readFile)(docPath, "utf8");
                }
                catch {
            ...
            if (method === "GET" && path === "/view/pixel-state.json") {
                ...
                return writeJson(res, 200, {
                    ok: true,
                    state: (0, pixel_state_1.buildPixelState)(snapshot),
                });
            }
            if (method === "GET" &&
                ...
                const snapshot = await readReadModelSnapshot();
                const exportPayload = await (0, export_bundle_1.buildExportBundle)(snapshot, "api", requestId);
                let exportSnapshot;
                let backupExport;
                try {
                    [exportSnapshot, backupExport] = await Promise.all([
                        (0, replay_index_1.writeExportSnapshot)(exportPayload, requestId),
                        (0, export_bundle_1.writeExportBundle)(exportPayload, "backup"),
                    ]);
            ...
            if (method === "GET" && (path === "/done-checklist" || path === "/api/done-checklist")) {
                assertAllowedQueryParams(url.searchParams, [], true);
                const snapshot = await readReadModelSnapshot();
                const checklist = await (0, done_checklist_1.buildDoneChecklist)(snapshot);
                return writeJson(res, 200, {
                    ok: true,
                    checklist,
                });
            ...
            if (method === "GET" && path === "/api/docs") {
                ...
                return writeJson(res, 200, {
                    ok: true,
                    docs: (0, api_docs_1.buildApiDocs)(),
                });
            }
            ...
            if (method === "PATCH" && path === "/api/ui/preferences") {
                ...
                const current = await (0, ui_preferences_1.loadUiPreferences)();
                const merged = mergeUiPreferencesPatch(current.preferences, payload);
                const saved = await (0, ui_preferences_1.saveUiPreferences)(merged);
                return writeJson(res, 200, {
                    ok: true,
                    path: saved.path,
            ...
            if (method === "GET" && path === "/api/search/tasks") {
                ...
                const snapshot = await readReadModelSnapshot();
                const matches = (0, task_store_1.listTasks)(snapshot.tasks, projectTitleMap(snapshot))
                    .filter((task) => safeSubstringMatch(query.q, task.taskId, task.title, task.owner, task.projectId, task.projectTitle, task.dueAt, task.status));
            ...
            if (method === "GET" && path === "/api/search/projects") {
                ...
                const matches = snapshot.projects.projects
                    .filter((project) => safeSubstringMatch(query.q, project.projectId, project.title, project.owner, project.status));
            ...
            if (method === "GET" && path === "/api/search/sessions") {
                ...
                const matches = snapshot.sessions
                    .filter((session) => safeSubstringMatch(query.q, session.sessionKey, session.label, session.agentId, session.state, session.lastMessageAt));
            ...
            if (method === "GET" && path === "/api/search/exceptions") {
                ...
                const matches = feed.items
                    .filter((item) => safeSubstringMatch(query.q, item.level, item.code, item.source, item.sourceId, item.route, item.message));
            ...
            if (method === "GET" && path === "/api/usage-cost") {
                assertAllowedQueryParams(url.searchParams, [], true);
                const snapshot = await readReadModelSnapshot();
                const usage = await (0, usage_cost_1.buildUsageCostSnapshot)(snapshot);
                return writeJson(res, 200, {
                    ok: true,
                    usage,
                });
            ...
            if (method === "GET" && path === "/api/subscription/template") {
                ...
                return writeJson(res, 200, {
                    ...
                    hint: `Save as ${(0, node_path_1.join)(process.cwd(), "runtime", "subscription-snapshot.json")}`,
                });
            }
            if (method === "GET" && path === "/api/tasks/heartbeat") {
                ...
                const runs = await (0, task_heartbeat_1.readTaskHeartbeatRuns)(limit);
                return writeJson(res, 200, {
                    ok: true,
                    path: runs.path,
                    count: runs.count,
            ...
            if (method === "POST" && path === "/api/tasks/heartbeat") {
                ...
                if (maxTasksPerRun !== undefined)
                    gate.maxTasksPerRun = maxTasksPerRun;
                const result = await (0, task_heartbeat_1.runTaskHeartbeat)({ gate });
            ...
            if (method === "GET" && path === "/api/replay/index") {
                ...
                const replay = await (0, replay_index_1.loadReplayIndex)({
                    timelineLimit: readPositiveIntQuery(url.searchParams.get("timelineLimit"), "timelineLimit", 80, true, 400),
            ...
            if (method === "POST" && path === "/api/import/dry-run") {
                ...
                if (typeof payload.fileName === "string" && payload.fileName.trim() !== "") {
                    validation = await (0, import_dry_run_1.validateExportFileDryRun)(payload.fileName);
                }
                else if (payload.bundle !== undefined) {
                    validation = (0, import_dry_run_1.validateExportBundleDryRun)(payload.bundle, "payload.bundle");
                }
                else {
                    validation = (0, import_dry_run_1.validateExportBundleDryRun)(payload, "payload");
                }
                await (0, operation_audit_1.appendOperationAudit)({
            ...
            if (method === "POST" && path === "/api/import/live") {
                ...
                const result = await (0, import_live_1.applyImportMutation)({
            ...
            if (method === "GET" && (path === "/projects" || path === "/api/projects")) {
                const projectStore = await (0, project_store_1.loadProjectStore)();
                const filters = parseProjectFilters(url.searchParams, path === "/api/projects");
                const projects = applyProjectFilters((0, project_store_1.listProjects)(projectStore), filters);
                return writeJson(res, 200, {
                    ok: true,
            ...
            if (method === "POST" && path === "/api/projects") {
                ...
                const created = await (0, project_store_1.createProject)(payload);
            ...
            if (method === "PATCH" && path.startsWith("/api/projects/")) {
                ...
                const updated = await (0, project_store_1.updateProject)({
                    ...payload,
                    projectId,
                });
            ...
            if (method === "GET" && (path === "/tasks" || path === "/api/tasks")) {
                ...
                const allTasks = (0, task_store_1.listTasks)(snapshot.tasks, projectTitleMap(snapshot));
            ...
            if (method === "POST" && path === "/api/tasks") {
                ...
                const created = await (0, task_store_1.createTask)(payload);
                return writeJson(res, 201, { ok: true, ...created });
            }
            if (method === "PATCH" && path.startsWith("/api/tasks/") && path.endsWith("/status")) {
                ...
                const updated = await (0, task_store_1.updateTaskStatus)({
                    taskId,
            ...
            if (method === "GET" && (path === "/sessions" || path === "/api/sessions")) {
                ...
                const sessions = await (0, session_conversations_1.listSessionConversations)({
                    snapshot,
                    client: toolClient,
            ...
            if (method === "GET" && path.startsWith("/api/sessions/")) {
                ...
                const detail = await (0, session_conversations_1.getSessionConversationDetail)({
                    snapshot,
                    client: toolClient,
                    sessionKey,
                    historyLimit,
            ...
            if (method === "GET" && path.startsWith("/details/task/")) {
                ...
                const linkedSessions = linkedSessionItems.map((detail) => ({
            ...
            if (method === "GET" && path.startsWith("/details/cron/")) {
                ...
                const overview = await (0, cron_overview_1.buildCronOverview)(snapshot, config_1.POLLING_INTERVALS_MS.cron);
            ...
            if (method === "GET" && path === "/api/audit") {
                ...
                const timeline = (0, audit_timeline_1.filterAuditTimeline)(await (0, audit_timeline_1.loadAuditTimeline)(snapshot), severity);
                return writeJson(res, 200, {
                    ok: true,
                    severity,
                    timeline,
            ...
            if (method === "GET" && path === "/exceptions") {
                ...
                return writeJson(res, 200, {
                    ok: true,
                    feed: (0, commander_1.commanderExceptionsFeed)(snapshot),
                });
            }
            if (method === "GET" && path === "/notifications/preview") {
                ...
                const snapshot = await readReadModelSnapshot();
                const feed = (0, commander_1.commanderExceptionsFeed)(snapshot);
                const policy = await (0, notification_policy_1.loadNotificationPolicy)();
                const preview = (0, notification_policy_1.buildNotificationPreview)(feed, policy, evaluatedAt);
            ...
            if (method === "GET" && path === "/healthz") {
                assertAllowedQueryParams(url.searchParams, [], true);
                const snapshot = await readReadModelSnapshot();
                const health = await (0, healthz_1.buildHealthzPayload)(snapshot);
            ...
            if (method === "GET" && path === "/digest/latest") {
                ...
                const latest = await (0, digest_renderer_1.loadLatestDigest)();
                return writeText(res, 200, (0, digest_renderer_1.renderLatestDigestPage)(latest), "text/html; charset=utf-8");
            }
            ...
            if (method === "GET" && path === "/api/action-queue/acks/prune-preview") {
                ...
                assertAllowedQueryParams(url.searchParams, [], true);
                const preview = await (0, notification_center_1.previewStaleAcksPrune)();
                return writeJson(res, 200, { ok: true, preview });
            }
            if (method === "POST" && path.startsWith("/api/action-queue/") && path.endsWith("/ack")) {
                ...
                const snapshot = await readReadModelSnapshot();
                const queue = await readNotificationCenter(snapshot);
                const acknowledged = await (0, notification_center_1.acknowledgeActionQueueItem)({
                    itemId,
                    note: payload.note,
                    ttlMinutes,
                    snoozeUntil,
            ...
            if (method === "POST" && path === "/action-queue/ack") {
                ...
                const snapshot = await readReadModelSnapshot();
                const queue = await readNotificationCenter(snapshot);
                await (0, notification_center_1.acknowledgeActionQueueItem)({ itemId }, queue);
            ...
            if (error instanceof task_store_1.TaskStoreValidationError ||
                error instanceof project_store_1.ProjectStoreValidationError ||
                error instanceof notification_center_1.NotificationCenterValidationError) {
    ...
    server.listen(port, host, () => {
        console.log(`[mission-control] ui listening at http://${host}:${port}`);
...
async function readSnapshotRaw() {
    try {
        return await (0, promises_1.readFile)(SNAPSHOT_PATH, "utf8");
    }
    catch {
...
async function readOptionalFileStamp(path) {
    try {
        const file = await (0, promises_1.stat)(path);
...
async function readReadModelSourceStamp() {
    const parts = await Promise.all([
        ...
        readOptionalFileStamp(project_store_1.PROJECTS_PATH),
        readOptionalFileStamp(task_store_1.TASKS_PATH),
        readOptionalFileStamp(budget_policy_1.BUDGET_POLICY_PATH),
    ]);
    return parts.join("|");
}
async function readReadModelSnapshot() {
    ...
    const nextValue = (async () => {
        ...
        const [projects, tasks, budgetPolicy] = await Promise.all([
            (0, project_store_1.loadProjectStore)(),
            (0, task_store_1.loadTaskStore)(),
            (0, budget_policy_1.loadBudgetPolicy)(),
        ]);
        ...
        const value = {
            ...
            projects,
            projectSummaries: (0, project_summary_1.computeProjectSummaries)(projects, tasks),
            tasks,
            tasksSummary: (0, task_summary_1.computeTasksSummary)(tasks, projects.projects.length),
            budgetSummary: (0, budget_governance_1.computeBudgetSummary)(sessions, statuses, tasks, projects, budgetPolicy.policy),
            generatedAt: typeof snapshot.generatedAt === "string" && !Number.isNaN(Date.parse(snapshot.generatedAt))
...
async function readReadModelSnapshotWithLiveSessions(toolClient) {
    ...
    try {
        ...
        const sessions = (0, openclaw_mappers_1.mapSessionsListToSummaries)(live);
...
async function readNotificationCenter(snapshot) {
    const exceptionsFeed = (0, commander_1.commanderExceptionsFeed)(snapshot);
    const acks = await (0, notification_center_1.loadAcksStore)();
    const linksByItemId = (0, action_queue_links_1.buildActionQueueLinks)(exceptionsFeed, snapshot);
    return (0, notification_center_1.buildNotificationCenter)(exceptionsFeed, acks, linksByItemId);
}
...
function sanitizeCronPurposeText(input, language, maxLength = 72) {
    ...
    if (lower.includes("run exactly one command via exec tool") ||
        lower.includes("cd /") ||
        lower.includes("&&") ||
...
function summarizeCronCommandPurpose(command, language) {
    ...
    if (normalized.includes("curl"))
        return pickUiText(language, "Fetch external data and update status.", "拉取外部数据并更新状态。");
    return pickUiText(language, "Run scheduled command.", "执行定时命令。");
}
...
function cronRuntimePurpose(jobId, language) {
    ...
    if (id.includes("task-heartbeat")) {
        return pickUiText(language, "Scan assigned backlog and trigger heartbeat pickup.", "扫描已分配待办任务，并按心跳规则推进。");
    }
    if (id.includes("monitor")) {
        return pickUiText(language, "Refresh runtime snapshot and keep dashboard state updated.", "刷新运行时快照，保持控制中心数据更新。");
    }
...
async function loadOpenclawCronCatalog(language) {
    for (const candidate of [...new Set(OPENCLAW_CRON_JOBS_CANDIDATES)]) {
        try {
            const raw = JSON.parse(await (0, promises_1.readFile)(candidate, "utf8"));
...
async function countRecentToolCalls(snapshot, toolClient) {
    if (!Array.isArray(snapshot.sessions) || snapshot.sessions.length === 0)
        return 0;
    const recentSessions = await (0, session_conversations_1.listSessionConversations)({
        snapshot,
        client: toolClient,
        filters: {},
        page: 1,
    ...
    return recentSessions.items.reduce((sum, item) => {
        if (typeof item.toolEventCount === "number")
            return sum + item.toolEventCount;
...
async function buildGlobalVisibilityViewModel(snapshot, toolClient, language, input = {}) {
    const cronOverview = input.cronOverview ?? (await (0, cron_overview_1.buildCronOverview)(snapshot, config_1.POLLING_INTERVALS_MS.cron));
    ...
    const strongTaskEvidenceCount = typeof input.strongTaskEvidenceCount === "number" ? input.strongTaskEvidenceCount : Math.max(0, inProgressCount - blockedCount);
    ...
    const currentTasksCount = input.currentTasksCount ?? Math.max(inProgressCount + blockedCount, strongTaskEvidenceCount + followupTaskEvidenceCount + weakTaskEvidenceCount);
    ...
    const latestHeartbeatRun = (await (0, task_heartbeat_1.readTaskHeartbeatRuns)(1)).runs[0];
    const cronTaskName = enabledCronCount > 0
        ? pickUiText(language, `${enabledCronCount} jobs enabled`, `已启用 ${enabledCronCount} 个任务`)
        : pickUiText(language, "No timed jobs", "暂无定时任务");
    const cronOwner = enabledOpenclawCronJobs.length > 0
        ? summarizeNames(enabledOpenclawCronJobs.map((job) => job.owner), language, pickUiText(language, "Scheduler", "调度器"))
        : formatExecutorAgentLabel("system-cron", language);
    const cronPurpose = (enabledOpenclawCronJobs[0]?.purpose ? sanitizeCronPurposeText(enabledOpenclawCronJobs[0]?.purpose, language, 56) : "") ||
    ...
    const cronNextRun = nonHeartbeatRuntimeCronJobs.find((job) => job.enabled)?.nextRunAt ??
        cronOverview.nextRunAt ??
        pickUiText(language, "Not scheduled", "未排程");
    const heartbeatNextRun = heartbeatJobs.find((job) => job.enabled)?.nextRunAt ??
    ...
    const heartbeatLatestResult = heartbeatEnabled
        ? latestHeartbeatRun
            ? pickUiText(language, `Last heartbeat: selected ${latestHeartbeatRun.selected} tasks, started ${latestHeartbeatRun.executed}.`, `最近心跳：挑出 ${latestHeartbeatRun.selected} 个任务，启动 ${latestHeartbeatRun.executed} 个。`)
            : pickUiText(language, `Active heartbeat checks: ${enabledHeartbeatCount}.`, `已开启任务心跳：${enabledHeartbeatCount} 个。`)
        : pickUiText(language, "No heartbeat check yet.", "还没有任务心跳记录。");
    const heartbeatPurpose = pickUiText(language, "Check assigned tasks and start the picked ones.", "检查已分配任务，并启动挑中的任务。");
    ...
    const rows = [
        {
            ...
            latestResult: scheduleReady
                ? pickUiText(language, `Active timed jobs: ${enabledCronCount}.`, `已开启定时任务：${enabledCronCount} 个。`)
        ...
        },
        {
            ...
            currentAction: currentTasksCount > 0
                ? hasWeakEvidence
            ...
            latestResult: currentTasksCount > 0
                ? hasWeakEvidence
                    ? pickUiText(language, `${strongTaskEvidenceCount} confirmed live, ${followupTaskEvidenceCount} need follow-up, ${weakTaskEvidenceCount} need inspection.`, `${strongTaskEvidenceCount} 个已确认在跑，${followupTaskEvidenceCount} 个需跟进，${weakTaskEvidenceCount} 个需排查。`)
            ...
            nextAction: currentTasksCount > 0
                ? hasWeakEvidence
    ...
    return {
        ...
        noTaskMessage: pickUiText(language, "No timed jobs, heartbeat, current tasks, or tool calls yet.", "暂无定时任务、任务心跳、当前任务或工具调用。"),
        signalCounts: {
...
function renderGlobalVisibilityStrip(model, language) {
    ...
    const signalTotal = model.signalCounts.schedule +
...
function snapshotFreshnessSignal(generatedAt, language) {
    ...
    if (ageMs <= 10 * 60 * 1000) {
        return {
            ...
            status: "connected",
            detail: pickUiText(language, `Updated ${ageLabel}; suitable for deciding what is happening now.`, `更新于${ageLabel}，适合直接判断现在发生了什么。`),
        };
    }
    if (ageMs <= 30 * 60 * 1000) {
        return {
            ...
            label: pickUiText(language, "Live picture", "实时画面"),
            status: "partial",
            detail: pickUiText(language, `Updated ${ageLabel}; still useful, but not precise enough for second-by-second judgement.`, `更新于${ageLabel}，仍然有参考价值，但不适合做秒级判断。`),
        };
    }
    return {
        key: "freshness",
...
function usageCoverageStatus(usage) {
    if (usage.connectors.requestCounts === "connected" &&
        usage.connectors.providerAttribution !== "not_connected") {
        return "connected";
    }
    if (usage.connectors.requestCounts !== "not_connected" ||
...
function historyCoverageStatus(replay) {
    if (replay.timeline.entries.length > 0 && replay.digests.length > 0)
        return "connected";
    if (replay.timeline.entries.length > 0 ||
...
function buildInformationCertaintyModel(input) {
    ...
    const signals = [
        freshness,
        {
            ...
            detail: officeRoster.status === "connected"
        ...
        },
        {
            ...
            detail: officePresence.status === "connected"
        ...
        },
        {
            ...
            detail: usageCost.subscription.status === "connected"
        ...
        },
        {
            ...
            detail: historyStatus === "connected"
    ...
    const score = Math.round(signals.reduce((sum, signal) => sum + scoreCoverageStatus(signal.status) * (weights.get(signal.key) ?? 0), 0) /
    ...
    if (strengths.length === 0) {
        strengths.push(pickUiText(language, "At least the current dashboard structure is readable even while signals are still sparse.", "即使信号还稀疏，当前看板结构本身仍然可读。"));
    }
    if (gaps.length === 0) {
        gaps.push(pickUiText(language, "No obvious blind spot is standing out right now.", "当前没有明显突出的盲区。"));
    }
    if (score >= 80) {
        return {
            score,
      ...
      <details class="compact-table-details" style="margin-top:12px;" open>
        ...
        <div class="fold-body">
          <table>
            ...
            <tbody>${model.signals
        .map((signal) => `<tr><td>${escapeHtml(signal.label)}</td><td>${badge(signal.status, dataConnectionLabel(signal.status, language))}</td><td>${escapeHtml(signal.detail)}</td></tr>`)
        .join("")}</tbody>
          </table>
        </div>
      </details>
...
function hasFreshRuntimeTimestamp(value, nowMs, windowMs) {
    const parsed = toSortableMs(value);
    return parsed > 0 && nowMs - parsed <= windowMs;
}
function isStaleRuntimeTimestamp(value, nowMs, windowMs) {
    const parsed = toSortableMs(value);
    return parsed > 0 && nowMs - parsed > windowMs;
}
function countStalledRunningSessions(sessions, sessionItems, nowMs, windowMs = STALLED_RUNNING_SESSION_WINDOW_MS) {
    const latestBySessionKey = new Map(sessionItems.map((item) => [item.sessionKey, item.latestHistoryAt ?? item.lastMessageAt]));
...
function buildTaskCertaintyCards(input) {
    ...
    const pendingApprovalSessionKeys = new Set(input.approvals
    ...
    return input.tasks
        ...
        for (const sessionKey of linkedSessionKeys) {
            ...
            if (preview?.executionChain?.accepted || preview?.executionChain?.spawned)
                executionEvidenceCount += 1;
            if ((preview?.toolEventCount ?? 0) > 0 || preview?.latestKind === "tool_event")
                toolEvidenceCount += 1;
        }
        ...
        if (pendingApprovals > 0)
            score -= Math.min(18, pendingApprovals * 8);
        ...
        if (pendingApprovals > 0) {
            gaps.push(pickUiText(input.language, `${pendingApprovals} linked approval item(s) are still waiting.`, `还有 ${pendingApprovals} 个关联审批在等待处理。`));
        }
        if (waitingApprovalSessionCount > 0) {
        ...
        const tone = errorSessionCount > 0
            ? "blocked"
...
function renderTaskCertaintySection(cards, language = "zh") {
    ...
    return `<section class="card" id="task-certainty-board">
      ...
      <div class="decision-list">${cards
        .slice(0, 8)
        .map((card) => `<a class="decision-row" href="${escapeHtml(card.detailHref)}">
...
function mergeSessionConversationItems(primary, secondary) {
    ...
    for (const item of [...primary, ...secondary]) {
        if (!item.sessionKey.trim() || merged.has(item.sessionKey))
            continue;
...
async function loadSessionConversationItemsByKeys(snapshot, toolClient, sessionKeys, historyLimit) {
    ...
    if (normalizedKeys.length === 0)
        return [];
    const details = await Promise.all(normalizedKeys.map(async (sessionKey) => {
        const detail = await (0, session_conversations_1.getSessionConversationDetail)({
            snapshot,
            client: toolClient,
            sessionKey,
...
async function loadCachedSessionPreview(snapshot, toolClient) {
    ...
    const value = await (0, session_conversations_1.listSessionConversations)({
        snapshot,
        client: toolClient,
        filters: {},
        page: 1,
...
async function loadCachedUsageCost(snapshot, mode) {
    ...
    const nextValue = (0, usage_cost_1.buildUsageCostSnapshot)(snapshot, mode);
    if (mode === "full") {
...
async function loadCachedOfficeSessionPresence() {
    ...
    const value = await (0, office_session_presence_1.loadBestEffortOfficeSessionPresence)();
...
async function renderHtml(filters, toolClient, options) {
    ...
    const renderPhases = [];
    const markRenderPhase = (label) => {
        const now = performance.now();
        renderPhases.push(`${label}=${Math.round(now - renderPhaseAt)}ms`);
        renderPhaseAt = now;
    };
    const snapshot = await readReadModelSnapshotWithLiveSessions(toolClient);
    const t = (en, zh) => pickUiText(options.language, en, zh);
    const sectionLinks = dashboardSectionLinks(options.language);
    const activeSection = options.section === "office-space"
        ? "team"
        : options.section === "calendar"
            ? "projects-tasks"
            : options.section;
    const usageCostMode = "full";
    ...
    const sectionLeadText = activeSection === "overview"
        ? t("Decide from one screen: system health, items needing your intervention, who is active, and AI burn.", "一个首页只回答四件事：系统是否正常、哪里需要你介入、谁在忙、AI 用量是否异常。")
        : activeSection === "projects-tasks"
            ? t("Start with schedule and cron execution. Staff can be active from cron or ad-hoc sessions even when there is no tracked task row yet.", "先看排程和 Cron 执行。员工显示在工作，可能只是 Cron 或临时会话在跑，不一定已经落成可跟踪的任务条目。")
            : sectionMeta.blurb;
    ...
    const [cronOverview, openclawCronJobs, replayPreview, usageCost, officeRoster, officePresence] = await Promise.all([
        (0, cron_overview_1.buildCronOverview)(snapshot, config_1.POLLING_INTERVALS_MS.cron),
        loadOpenclawCronCatalog(options.language),
        (0, replay_index_1.loadReplayIndex)({
            timelineLimit: 20,
            digestLimit: 10,
            exportLimit: 10,
        }),
        loadCachedUsageCost(snapshot, usageCostMode),
        (0, agent_roster_1.loadBestEffortAgentRoster)(),
    ...
    const [teamSnapshot, memoryFiles, memoryFacetOptions, workspaceFiles, workspaceFacetOptions, taskEvidenceItems] = await Promise.all([
        ...
        needsMemoryFiles ? listEditableFiles("memory") : Promise.resolve([]),
        needsMemoryFiles ? listMemoryFacetOptions() : Promise.resolve([]),
        needsWorkspaceFiles ? listEditableFiles("workspace") : Promise.resolve([]),
        needsWorkspaceFiles ? listWorkspaceFacetOptions() : Promise.resolve([]),
        needsTaskEvidence
            ? loadCachedTaskEvidenceSessions(snapshot, toolClient, tasks.flatMap((task) => task.sessionKeys), 24)
            : Promise.resolve([]),
    ]);
    ...
    const heartbeatRuns = await (0, task_heartbeat_1.readTaskHeartbeatRuns)(1);
    const latestHeartbeatRun = heartbeatRuns.runs[0];
    const currentTaskHealth = taskCertaintyWeakCount === 0 ? "ok" : "warn";
    const mappingTaskHint = controlCenterMappingTasks.length > 0
        ? t(`${controlCenterMappingTasks.length} board-only mapping examples are hidden from execution metrics.`, `另有 ${controlCenterMappingTasks.length} 个看板样例（不执行任务）。`)
        : "";
    ...
    const toolRows = toolSessions.length === 0
        ...
        : toolSessions
            ...
            .join("");
    const importGuard = (0, import_live_1.readImportMutationGuardState)();
    const tokenGateStatus = config_1.LOCAL_TOKEN_AUTH_REQUIRED
    ...
    const importGuardRows = [
        {
            label: "只读保护",
            value: String(config_1.READONLY_MODE),
            note: config_1.READONLY_MODE ? "当前只允许安全演练，不会写入真实变更。" : "允许真实写入，请确认后使用。",
            status: config_1.READONLY_MODE ? "enabled" : "warn",
        },
        {
            label: "关键操作身份验证",
            value: String(config_1.LOCAL_TOKEN_AUTH_REQUIRED),
            note: config_1.LOCAL_TOKEN_AUTH_REQUIRED ? "已开启，关键操作需要身份验证。" : "未开启，建议在生产环境开启。",
            status: config_1.LOCAL_TOKEN_AUTH_REQUIRED ? "enabled" : "warn",
        },
        {
            label: "身份验证配置",
        ...
        {
            label: "变更写入开关",
            value: String(config_1.IMPORT_MUTATION_ENABLED),
            note: config_1.IMPORT_MUTATION_ENABLED ? "允许写入导入变更。" : "已关闭导入写入。",
            status: config_1.IMPORT_MUTATION_ENABLED ? "warn" : "disabled",
        },
        {
            label: "审批写入开关",
            value: String(config_1.APPROVAL_ACTIONS_ENABLED),
            note: config_1.APPROVAL_ACTIONS_ENABLED ? "允许执行审批写入。" : "已关闭审批写入。",
            status: config_1.APPROVAL_ACTIONS_ENABLED ? "warn" : "disabled",
        },
        {
            label: "默认保护模式",
            value: importGuard.defaultMode,
            note: importGuard.defaultMode === "blocked"
                ? "当前为保护状态，仅允许演练。"
        ...
        .map((item) => `<tr><td>${escapeHtml(item.label)}</td><td>${badge(item.status)}</td><td>${escapeHtml(item.value)}</td><td>${escapeHtml(item.note)}</td></tr>`)
        .join("");
    ...
    const replayRows = replayRowItems
        ...
        .join("");
    const replayMetricsHtml = replayRowItems.length === 0
    ...
    const approvalsPreviewMeta = allApprovals.length > topApprovals.length
    ...
    const approvalsItems = topApprovals.length === 0
        ...
        : topApprovals
            ...
            .join("");
    const budgetItems = nonOkBudgets.length === 0
    ...
    const taskRows = tasks.length === 0
        ...
        : tasks
            .slice(0, 50)
            .map((task) => `<tr><td>${escapeHtml(task.projectTitle)}</td><td><code>${escapeHtml(task.taskId)}</code></td><td>${escapeHtml(task.title)}</td><td>${badge(task.status, taskStateLabel(task.status, options.language))}</td><td>${escapeHtml(task.owner)}</td><td>${escapeHtml(task.dueAt ?? "-")}</td><td>${escapeHtml(task.updatedAt)}</td></tr>`)
            .join("");
    const taskGroupedListHtml = tasks.length === 0
    ...
    const toolGroupedListHtml = toolSessions.length === 0
        ...
        : `<div class="group-list"><details class="group-section" open><summary>${escapeHtml(t("Active tool sessions", "活跃工具会话"))} (${toolSessions.length})</summary><ul class="group-items">${toolSessions
            ...
            .join("")}</ul></details></div>`;
    const heartbeatGroupedListHtml = heartbeatJobs.length === 0
        ...
        : `<div class="group-list"><details class="group-section" open><summary>${escapeHtml(t("Heartbeat checks", "心跳检查项"))} (${heartbeatJobs.length})</summary><ul class="group-items">${heartbeatJobs
            ...
            .join("")}</ul></details></div>`;
    const agentJobGroupedListHtml = agentJobCatalogRows.length === 0
    ...
    const subscriptionWindowHint = usageCost.subscription.primaryWindowLabel || usageCost.subscription.secondaryUsedPercent !== undefined
        ? `${normalizeQuotaWindowLabel(usageCost.subscription.primaryWindowLabel, "5h")} / ${normalizeQuotaWindowLabel(usageCost.subscription.secondaryWindowLabel, "Week")}`
    ...
    const executiveCardsHtml = `<section class="executive-grid">${executiveCards
        .map((item) => `<article class="exec-card"><div class="exec-title">${escapeHtml(item.title)}</div><div class="exec-metric">${escapeHtml(item.metric)}</div><div class="meta">${escapeHtml(item.detail)}</div></article>`)
        .join("")}</section>`;
    const overviewTopMetrics = [
        ...
        },
        {
            ...
            detail: runtimeSessionIssueCount > 0
        ...
        },
        {
            ...
            detail: stalledRunningSessionCount > 0
        ...
        },
        {
            ...
            detail: usageToday?.sourceStatus === "not_connected"
                ? t("Not connected", "未连接")
    ...
    const overviewTopMetricHtml = `<section class="overview-kpi-grid">${overviewTopMetrics
        .map((item) => {
        const counterAttrs = typeof item.numericValue === "number"
    ...
    const replayMomentsRows = replayMoments.length === 0
    ...
    const runtimeTokenRangeLabel = usageToday?.sourceStatus === "not_connected"
    ...
    const usageSourcePieHtml = selectedUsageBreakdown.byAgent.length === 0 && selectedUsageBreakdown.byProject.length === 0
        ? ""
        : `<div class="bars">
          <div>
    ...
    const usageSessionTypeShareHtml = usageSessionTypeRows.length === 0
    ...
    const usageCronJobShareHtml = usageCronJobRows.length === 0
    ...
    const usageCronAgentShareHtml = usageCronAgentRows.length === 0
    ...
    const usageBudgetHeadline = usageCost.budget.status === "not_connected"
    ...
    const usageBudgetMeta = usage30d?.sourceStatus === "not_connected"
    ...
    const hasUsageActivity = usageCost.contextWindows.length > 0 ||
        usageCost.periods.some((item) => item.tokens > 0 || item.estimatedCost > 0 || item.statusSamples > 0);
    const usageContextHtml = usageCost.contextWindows.length === 0
    ...
    const usageSourceHtml = selectedUsageBreakdown.byAgent.length === 0 && selectedUsageBreakdown.byProject.length === 0
    ...
    const usageTaskHtml = usageTaskBreakdownRows.length === 0
    ...
    const usageAttributionHtml = selectedUsageBreakdown.byAgent.length === 0 && usageTaskBreakdownRows.length === 0
    ...
    const usageModelMixHtml = selectedUsageBreakdown.byModel.length === 0 && selectedUsageBreakdown.byProvider.length === 0
    ...
    const renderExecutionAgentItem = (item, mode) => {
    ...
    const taskRoleRows = taskRoleSummaries
        .map((item) => `<li><strong>${escapeHtml(item.owner)}</strong><div class="meta">${escapeHtml(t("Board labels", "看板标签"))} ${item.activeTasks} ${escapeHtml(t("items", "个"))} · ${escapeHtml(t("Examples", "示例"))}：${escapeHtml(item.sampleTaskIds.join("、") || t("None", "暂无"))}</div></li>`)
        .join("");
    const mappingTaskRows = controlCenterMappingTasks
        .map((task) => `<tr><td>${escapeHtml(task.taskId)}</td><td>${escapeHtml(task.owner)}</td><td>${badge(task.status, taskStateLabel(task.status, options.language))}</td></tr>`)
        .join("");
    const cronTable = allCronRows.length === 0
    ...
    const cronBoardHtml = allCronRows.length === 0
    ...
    const overviewActionItems = [
        ...
        },
        {
            ...
            detail: runtimeIssueCount > 0
                ? t(`${runtimeSessionIssueCount} blocked/error/waiting sessions · ${stalledRunningSessionCount} stalled runs`, `${runtimeSessionIssueCount} 个阻塞/异常/等待会话 · ${stalledRunningSessionCount} 个停滞执行`)
    ...
    const overviewPrimaryDirective = pendingDecisionCount > 0
    ...
    const overviewFocusScore = Math.max(0, Math.min(100, Math.round(100 -
    ...
    const overviewUpcomingRows = calendarEvents
        .slice(0, 4)
        .map((item) => `<div class="decision-row">
        <div class="decision-row-copy">
    ...
    const overviewBusyAgents = executionAgentSummaries
        ...
        .slice(0, 3);
    const overviewBusyCardsHtml = overviewBusyAgents.length === 0
    ...
    const taskDecisionPreviewHtml = actionQueue.queue.length > 0
        ...
        <details class="card compact-details" id="heartbeat-health">
          ...
          <div class="fold-body">
            ...
            <details class="compact-table-details" style="margin-top:12px;">
              ...
              <div class="fold-body">${heartbeatJobs.length === 0
        ? `<div class="empty-state">${escapeHtml(t("No heartbeat timed jobs found yet.", "尚未发现心跳定时任务。"))}</div>`
  ...
  `;
    ...
    const calendarBoardHtml = calendarBuckets.size === 0
        ...
        : `<div class="calendar-grid">${[...calendarBuckets.entries()]
            ...
            const rows = events
                .slice(0, 10)
                .map((item) => `<li class="calendar-event">
  ...
  `;
    const teamMembersTableRows = teamSnapshot.members.length === 0
        ? `<tr><td colspan="5">${escapeHtml(t("No staff found.", "暂无员工。"))}</td></tr>`
        : teamSnapshot.members
            .map((member) => `<tr><td>${escapeHtml(member.displayName)}</td><td><code>${escapeHtml(member.agentId)}</code></td><td>${escapeHtml(member.model)}</td><td>${escapeHtml(member.toolsProfile)}</td><td>${escapeHtml(member.workspace)}</td></tr>`)
            .join("");
    const teamSection = `
    <section class="card">
  ...
  `;
    ...
    <details class="card compact-details">
      ...
      <div class="fold-body">
        ...
        <div class="meta">${escapeHtml(t("Daily burn", "日均消耗"))} ${usageCost.budget.burnRatePerDay !== undefined
        ? formatCurrency(usageCost.budget.burnRatePerDay)
        : t("Data source not connected", "数据源未连接")}</div>
        <div class="meta">${escapeHtml(t("Estimated days remaining", "预计剩余天数"))} ${usageCost.budget.projectedDaysToLimit !== undefined
  ...
  `;
    ...
    const trackedTaskSummaryText = hasTrackedTaskPanels
        ? t(`Tracked tasks ${taskCertaintyCards.length} · Follow-up ${pendingDecisionCount}`, `跟踪任务 ${taskCertaintyCards.length} · 待处理 ${pendingDecisionCount}`)
        : t("No tracked task rows yet", "还没有跟踪任务条目");
    const trackedTaskExplanation = hasTrackedTaskPanels
        ? t("This lower-priority area is only for tracked task rows, decisions, and runtime evidence.", "这块低优先级区域只看可跟踪任务条目、待处理事项和运行证据。")
        : liveSessionCount > 0
            ? t("Staff status comes from live sessions. Cron jobs, heartbeat, and ad-hoc sessions can keep agents busy before anything becomes a tracked task row.", "员工状态来自实时会话。Cron、心跳和临时会话可能已经让智能体在工作，但还没有形成可跟踪的任务条目。")
            : t("There is no tracked task row visible right now. Start here only when you actually use the task store.", "当前还没有可见的跟踪任务条目。只有真正使用任务库时，这里才会出现内容。");
  ...
  `;
    ...
    <details class="card compact-details">
      ...
      <div class="fold-body">${topApprovals.length === 0
        ? `<div class="empty-state">${escapeHtml(t("No approval requests yet.", "暂无审批请求。"))}</div>`
        : `<div class="meta">${escapeHtml(approvalsPreviewMeta)}</div><ul class="story-list">${approvalsItems}</ul>`}</div>
    </details>
    <details class="card compact-details">
      <summary>${escapeHtml(t("Budget watch", "预算监控"))}</summary>
      <div class="fold-body">${nonOkBudgets.length === 0
        ? `<div class="empty-state">${escapeHtml(t("Budget status looks healthy right now.", "预算状态健康，当前无预警。"))}</div>`
        : `<table><thead><tr><th>${escapeHtml(t("Status", "状态"))}</th><th>${escapeHtml(t("Scope", "范围"))}</th><th>${escapeHtml(t("Target", "对象"))}</th><th>${escapeHtml(t("Usage", "使用情况"))}</th></tr></thead><tbody>${budgetItems}</tbody></table>`}</div>
    </details>
  `;
    const replaySection = `
    <section class="card">
  ...
  `;
    ...
    const globalVisibilityQuickRows = [
        ...
        .join("");
    const sidebarSignalRows = options.section === "overview"
  ...
  <div class="app-shell">
    ...
    <aside class="sidebar inspector-sidebar">
      ...
      <div class="card" style="margin-top:10px;">
        ...
        <div class="meta">${escapeHtml(t("Today's AI usage", "今日 AI 用量"))}：${usageToday?.sourceStatus === "not_connected"
        ? t("Data source not connected", "数据源未连接")
        : formatInt(usageToday?.tokens ?? 0)}</div>
        <div class="meta">${escapeHtml(t("Today's cost", "今日费用"))}：${usageToday?.sourceStatus === "not_connected"
        ? t("Data source not connected", "数据源未连接")
        : formatCurrency(usageToday?.estimatedCost ?? 0)}</div>
        <div class="meta">${escapeHtml(t("Last 30 days", "近 30 天"))}：${usage30d?.sourceStatus === "not_connected"
        ? t("Data source not connected", "数据源未连接")
        : formatCurrency(usage30d?.estimatedCost ?? 0)}</div>
...
function parseTaskFilters(searchParams, strict) {
    ...
    if (quick) {
        if ((0, ui_preferences_1.isUiQuickFilter)(quick)) {
            filters.quick = quick;
        }
        else if (strict) {
            throw new RequestValidationError("quick must be one of: all, attention, todo, in_progress, blocked, done", 400);
        }
    }
...
function parseProjectFilters(searchParams, strict) {
    ...
    const status = normalizeQueryString(searchParams.get("status"), "status", 30, strict);
    if (status) {
        if (project_store_1.PROJECT_STATES.includes(status)) {
            filters.status = status;
        }
        else if (strict) {
...
function parseSessionQuery(searchParams, strict) {
    ...
    if (state) {
        if (SESSION_STATES.includes(state)) {
            ...
            throw new RequestValidationError("state must be one of: idle, running, blocked, waiting_approval, error", 400);
        }
    }
...
function resolveDashboardSearchQuery(searchParams) {
    ...
    const scope = DASHBOARD_SEARCH_SCOPES.includes(rawScope)
        ? rawScope
        : "tasks";
...
function resolveDashboardSection(searchParams) {
    ...
    if (value === "alerts" || value === "replay-audit")
        return "overview";
    return DASHBOARD_SECTIONS.includes(value) ? value : "overview";
}
function resolveLegacyDashboardSection(path) {
    const section = LEGACY_DASHBOARD_ROUTE_SECTION[path];
    return section;
}
function resolveLegacyDashboardAnchor(path) {
    return LEGACY_DASHBOARD_ROUTE_ANCHOR[path];
}
function resolveLegacyDashboardSectionForSmoke(path) {
...
function buildDashboardSearchResult(snapshot, query) {
    ...
    if (query.scope === "tasks") {
        const result = buildBoundedSearchResult((0, task_store_1.listTasks)(snapshot.tasks, projectTitleMap(snapshot))
            .filter((task) => safeSubstringMatch(query.q, task.taskId, task.title, task.owner, task.projectId, task.projectTitle, task.status, task.dueAt)), query.limit);
        const items = result.items;
        const rows = items.length === 0
            ? "<tr><td colspan=\"6\">未找到匹配任务。</td></tr>"
            : items
                .map((item) => `<tr><td><code>${escapeHtml(item.taskId)}</code></td><td>${escapeHtml(item.title)}</td><td>${badge(item.status)}</td><td>${escapeHtml(item.owner)}</td><td>${escapeHtml(item.projectTitle)}</td><td>${escapeHtml(item.updatedAt)}</td></tr>`)
                .join("");
        return {
            scope: query.scope,
            q: query.q,
    ...
    if (query.scope === "projects") {
        const result = buildBoundedSearchResult(snapshot.projects.projects
            .filter((project) => safeSubstringMatch(query.q, project.projectId, project.title, project.owner, project.status)), query.limit);
        const items = result.items;
        const rows = items.length === 0
            ? "<tr><td colspan=\"5\">未找到匹配项目。</td></tr>"
            : items
                .map((item) => `<tr><td><code>${escapeHtml(item.projectId)}</code></td><td>${escapeHtml(item.title)}</td><td>${badge(item.status)}</td><td>${escapeHtml(item.owner)}</td><td>${escapeHtml(item.updatedAt)}</td></tr>`)
                .join("");
        return {
            scope: query.scope,
            q: query.q,
    ...
    if (query.scope === "sessions") {
        const result = buildBoundedSearchResult(snapshot.sessions
            .filter((session) => safeSubstringMatch(query.q, session.sessionKey, session.label, session.agentId, session.state, session.lastMessageAt)), query.limit);
        const items = result.items;
        const rows = items.length === 0
            ? "<tr><td colspan=\"5\">未找到匹配会话。</td></tr>"
            : items
                .map((item) => `<tr><td><code>${escapeHtml(item.sessionKey)}</code></td><td>${badge(item.state)}</td><td>${escapeHtml(item.agentId ?? "-")}</td><td>${escapeHtml(item.label ?? "-")}</td><td>${escapeHtml(item.lastMessageAt ?? "-")}</td></tr>`)
                .join("");
        return {
            scope: query.scope,
            q: query.q,
    ...
    const result = buildBoundedSearchResult((0, commander_1.commanderExceptionsFeed)(snapshot).items.filter((item) => safeSubstringMatch(query.q, item.level, item.code, item.source, item.sourceId, item.route, item.message)), query.limit);
    const items = result.items;
    const rows = items.length === 0
        ? "<tr><td colspan=\"5\">未找到匹配告警。</td></tr>"
        : items
            .map((item) => `<tr><td>${badge(item.level)}</td><td>${escapeHtml(item.code)}</td><td><code>${escapeHtml(item.sourceId)}</code></td><td>${escapeHtml(item.route)}</td><td>${escapeHtml(item.message)}</td></tr>`)
            .join("");
    return {
        scope: query.scope,
        q: query.q,
...
function renderDashboardSearchResult(result, language = "zh") {
    if (!result) {
        return `<div class="meta" style="margin-top:8px;">${escapeHtml(pickUiText(language, "Enter a keyword to search by scope.", "输入关键词后可按范围搜索。"))}</div>`;
    }
    const summary = result.count > result.returned
        ? pickUiText(language, `Scope: ${searchScopeLabel(result.scope, language)} · Keyword: ${result.q} · Showing ${result.returned} of ${result.count} matches`, `范围：${searchScopeLabel(result.scope, language)} · 关键词：${result.q} · 当前显示 ${result.returned}/${result.count} 条命中`)
        : pickUiText(language, `Scope: ${searchScopeLabel(result.scope, language)} · Keyword: ${result.q} · Matches: ${result.count}`, `范围：${searchScopeLabel(result.scope, language)} · 关键词：${result.q} · 命中：${result.count}`);
    return `<div class="meta" style="margin-top:8px;">${escapeHtml(summary)}</div>${result.rows}`;
}
function buildDashboardSearchResultForSmoke(snapshot, query) {
...
function resolveUiLanguage(searchParams, fallback) {
    ...
    return (0, ui_preferences_1.isUiLanguage)(normalized) ? normalized : fallback;
}
function mergeUiPreferencesPatch(current, payload) {
    ...
    if (payload.language !== undefined) {
        ...
        if (!(0, ui_preferences_1.isUiLanguage)(normalizedLanguage)) {
    ...
    if (payload.quickFilter !== undefined) {
        ...
        if (!(0, ui_preferences_1.isUiQuickFilter)(quick)) {
            throw new RequestValidationError("quickFilter must be one of: all, attention, todo, in_progress, blocked, done", 400);
        }
    ...
    if (payload.taskFilters !== undefined) {
        ...
        if (filtersObj.status !== undefined) {
            ...
            else if (typeof filtersObj.status === "string" && TASK_STATES.includes(filtersObj.status)) {
...
function normalizeDocCategoryKey(value) {
...
function resolveStaffWorkspaceRoot(member) {
    ...
    if (workspace && workspace !== "未标注" && workspace !== "unlisted")
        return workspace;
    return (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "agents", member.agentId);
}
async function loadStaffRoleEvidence(member) {
    ...
    for (const fileName of STAFF_ROLE_EVIDENCE_FILE_CANDIDATES) {
        const raw = await safeReadTextFile((0, node_path_1.join)(workspaceRoot, fileName));
        if (raw?.trim())
            output.push(raw);
    }
    const openclawConfig = await safeReadTextFile(OPENCLAW_CONFIG_PATH);
    if (openclawConfig?.trim())
        output.push(openclawConfig);
...
async function resolveStaffRoleLabel(member, language = "zh") {
    ...
    const explicitRole = roleEvidence
        .map((entry) => extractLabeledField(entry, ["Role", "职责", "角色"]))
        .find((value) => value && value.trim().length > 0) ?? "";
    const explicitMission = roleEvidence
...
async function listFileEntries(dir) {
    try {
        const rows = await (0, promises_1.readdir)(dir, { withFileTypes: true });
        const files = rows.filter((row) => row.isFile());
        const result = [];
        for (const file of files) {
            const fullPath = (0, node_path_1.join)(dir, file.name);
            try {
                const meta = await (0, promises_1.stat)(fullPath);
                result.push({
                    name: file.name,
                    path: fullPath,
...
function normalizeEditableFileScope(value) {
    if (value === "memory" || value === "workspace")
        return value;
    return undefined;
}
async function buildEditableFileEntry(input) {
    try {
        const meta = await (0, promises_1.stat)(input.sourcePath);
        if (!meta.isFile() || meta.size > EDITABLE_TEXT_FILE_MAX_BYTES)
            return undefined;
        const raw = await safeReadTextFile(input.sourcePath);
        if (raw === undefined)
            return undefined;
        const ext = (0, node_path_1.extname)(input.sourcePath).toLowerCase();
        const relativePath = input.relativeBase
            ? (0, node_path_1.relative)(input.relativeBase, input.sourcePath) || (0, node_path_1.basename)(input.sourcePath)
            : (0, node_path_1.basename)(input.sourcePath);
        return {
            scope: input.scope,
            title: extractMarkdownHeading(raw) || (0, node_path_1.basename)(input.sourcePath, ext) || (0, node_path_1.basename)(input.sourcePath),
...
async function walkWorkspaceMarkdownFiles(root, currentDir = root) {
    try {
        const rows = await (0, promises_1.readdir)(currentDir, { withFileTypes: true });
        const output = [];
        for (const row of rows) {
            const fullPath = (0, node_path_1.join)(currentDir, row.name);
            if (row.isDirectory()) {
                if (WORKSPACE_EDITABLE_SKIP_DIRS.has(row.name))
                    continue;
            ...
            if (!row.isFile())
                continue;
            const ext = (0, node_path_1.extname)(row.name).toLowerCase();
...
async function listEditableMemoryFiles() {
    ...
    const append = async (entry) => {
        if (!entry)
            return;
        const key = (0, node_path_1.resolve)(entry.sourcePath);
        if (seen.has(key))
            return;
        seen.add(key);
        output.push(entry);
    ...
    const mainRootFiles = [
        (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "MEMORY.md"),
    ];
    for (const candidateFile of mainRootFiles) {
        await append(await buildEditableFileEntry({
            scope: "memory",
            category: "Main 长期记忆",
    ...
    const mainMemoryDir = (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "memory");
    const mainMemoryFiles = await listFileEntries(mainMemoryDir);
    for (const file of mainMemoryFiles) {
        const ext = (0, node_path_1.extname)(file.name).toLowerCase();
        if (!MEMORY_EDITABLE_EXTENSIONS.has(ext))
            continue;
        await append(await buildEditableFileEntry({
            scope: "memory",
            category: "Main 记忆记录",
    ...
    for (const scope of agentScopes) {
        ...
        for (const fileName of agentProfileFiles) {
            await append(await buildEditableFileEntry({
                ...
                sourcePath: (0, node_path_1.join)(scope.workspaceRoot, fileName),
                relativeBase: OPENCLAW_WORKSPACE_ROOT,
                facetKey: scope.facetKey,
                facetLabel: scope.facetLabel,
            }));
        }
        const agentMemoryDir = (0, node_path_1.join)(scope.workspaceRoot, "memory");
    ...
    return output.sort((a, b) => (a.facetKey === "main" ? -1 : b.facetKey === "main" ? 1 : (a.facetLabel ?? "").localeCompare(b.facetLabel ?? "", "zh-Hans-CN")) ||
...
async function listEditableWorkspaceFiles() {
    ...
    for (const relativePath of SHARED_DOCUMENT_FILE_CANDIDATES) {
        await append(await buildEditableFileEntry({
            scope: "workspace",
            category: "Main 核心文档",
            sourcePath: (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, relativePath),
            relativeBase: OPENCLAW_WORKSPACE_ROOT,
            facetKey: "main",
            facetLabel: "Main",
        }));
...
function documentFilePriority(relativePath) {
    const fileName = (0, node_path_1.basename)(relativePath).toLowerCase();
    const order = [
        "agents.md",
        "identity.md",
        "soul.md",
...
async function listEditableFiles(scope) {
    if (scope === "memory")
        return listEditableMemoryFiles();
...
async function loadEditableAgentScopesFromWorkspaceDirs() {
    ...
    const agentsRoot = (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "agents");
    try {
        const agentDirs = await (0, promises_1.readdir)(agentsRoot, { withFileTypes: true });
        for (const row of agentDirs) {
            ...
            output.push({
                ...
                workspaceRoot: (0, node_path_1.join)(agentsRoot, agentId),
            });
        }
    }
    catch {
...
function resolveEditableAgentScopesFromConfig(input) {
    ...
    for (const item of list) {
        ...
        const workspaceRoot = facetKey === "main"
            ...
            : (0, node_path_1.resolve)(asString(row.workspace)?.trim() || (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "agents", rawId));
        output.push({
            agentId: rawId,
            facetKey,
...
function resolveEditableAgentScopesFromConfigForSmoke(input) {
...
function buildMainEditableAgentScope() {
    return {
        agentId: "main",
        facetKey: "main",
        facetLabel: "Main",
...
function resolveEditableAgentScopesFromWorkspaceAgentIds(agentIds) {
    ...
    for (const rawId of agentIds) {
        ...
        scopes.push({
            ...
            facetKey,
            facetLabel: humanizeOperatorLabel(agentId),
            workspaceRoot: (0, node_path_1.join)(OPENCLAW_WORKSPACE_ROOT, "agents", agentId),
        });
    }
    return scopes.sort(compareEditableAgentScopes);
}
async function resolveEditableFileEntry(scope, sourcePath) {
    const target = (0, node_path_1.resolve)(sourcePath);
    const entries = await listEditableFiles(scope);
    return entries.find((entry) => (0, node_path_1.resolve)(entry.sourcePath) === target);
}
...
async function writeEditableFileContent(scope, sourcePath, content) {
    ...
    if (!entry)
        return undefined;
    await (0, promises_1.writeFile)(entry.sourcePath, content, "utf8");
...
async function loadMemoryEntries() {
    ...
    for (const candidateDir of MEMORY_DIR_CANDIDATES) {
        ...
        for (const file of files.slice(0, 36)) {
            ...
            if (ext !== ".md" && ext !== ".markdown" && ext !== ".txt")
                continue;
            if (file.size > 420 * 1024)
                continue;
            const raw = await safeReadTextFile(file.path);
            if (!raw)
                continue;
            const title = extractMarkdownHeading(raw) || (0, node_path_1.basename)(file.name, ext) || file.name;
    ...
    for (const candidateFile of LONG_TERM_MEMORY_FILE_CANDIDATES) {
        ...
        if (!raw)
            continue;
        const ext = (0, node_path_1.extname)(candidateFile).toLowerCase();
        const title = extractMarkdownHeading(raw) || (0, node_path_1.basename)(candidateFile, ext) || (0, node_path_1.basename)(candidateFile);
        longTerm.push({
            day: "长期",
            title,
...
async function loadDocHubEntries(chatEntries = []) {
    const output = [];
    for (const candidate of DOC_HUB_DIR_CANDIDATES) {
        ...
        for (const file of files.slice(0, 40)) {
            ...
            if (!raw)
                continue;
            const title = extractMarkdownHeading(raw) ||
                (0, node_path_1.basename)(file.name, ext) ||
                file.name;
            output.push({
                title,
                excerpt: toPlainSummary(raw, 180),
...
async function renderEditableFileWorkbench(input) {
    ...
    const localizeCategoryLabel = (value) => {
        ...
        if (!trimmed)
            return value;
        if (trimmed === "共享文档")
            return t("Shared docs", "共享文档");
        if (trimmed === "Main 长期记忆")
            return t("Main long-term memory", "Main 长期记忆");
        if (trimmed === "Main 记忆记录")
            return t("Main memory log", "Main 记忆记录");
    ...
    const discoveredFacetOptions = input.entries
        ...
        .reduce((acc, entry) => {
        if (!entry.facetKey || !entry.facetLabel)
            return acc;
    ...
    const defaultFacetKey = requestedDefaultFacet && facetOptions.some((item) => item.key === requestedDefaultFacet)
        ? requestedDefaultFacet
        : input.includeAllFacet === false
            ? facetOptions[0]?.key ?? "all"
            : "all";
    const firstEntry = input.entries.find((entry) => normalizeFacetKey(entry.facetKey) === defaultFacetKey) ?? input.entries[0];
    const initialContent = (await safeReadTextFile(firstEntry.sourcePath)) ?? "";
    const facetSwitcherHtml = facetOptions.length <= 1
        ? ""
        : `<div class="segment-switch file-facet-switch" data-file-facet-switch>
          ...
          ${facetOptions
            .map((item) => `<button class="segment-item${defaultFacetKey === item.key ? " active" : ""}" type="button" data-file-facet="${escapeHtml(item.key)}">${escapeHtml(localizeFacetLabel(item.label))}</button>`)
            .join("")}
        </div>`;
    const tokenHint = !config_1.LOCAL_TOKEN_AUTH_REQUIRED
    ...
    const tokenField = config_1.LOCAL_TOKEN_AUTH_REQUIRED && config_1.LOCAL_API_TOKEN
    ...
    <div class="file-workbench" data-file-editor-root data-scope="${escapeHtml(input.scope)}" data-language="${escapeHtml(input.language)}" data-default-facet="${escapeHtml(defaultFacetKey)}" data-token-required="${config_1.LOCAL_TOKEN_AUTH_REQUIRED ? "1" : "0"}" data-local-token-header="${escapeHtml(config_1.LOCAL_TOKEN_HEADER)}">
      <aside class="file-sidebar">
        ...
        <div class="file-nav" data-file-nav>
          ${input.entries
        .map((entry) => `<button class="file-nav-item${entry.sourcePath === firstEntry.sourcePath ? " active" : ""}" type="button" data-file-item data-source-path="${escapeHtml(entry.sourcePath)}" data-file-facet-key="${escapeHtml(normalizeFacetKey(entry.facetKey))}" data-file-search="${escapeHtml(`${entry.title} ${entry.relativePath} ${entry.category} ${entry.facetLabel ?? ""}`.toLowerCase())}">
...
function renderStructuredChatDocSummary(entries) {
    if (entries.length === 0) {
        return '<div class="empty-state">尚无聊天输出结构化入库记录。</div>';
    }
    return `<ul class="story-list">${entries
        .slice(0, 16)
        .map((entry) => `<li><strong>${escapeHtml(entry.title)}</strong><div class="meta">${escapeHtml(entry.excerpt)}</div><div class="meta">会话 ${escapeHtml(entry.sourceSessionKey)} · 更新 ${escapeHtml(entry.updatedAt)}</div></li>`)
        .join("")}</ul>`;
}
async function loadTeamSnapshot(officeRoster) {
    ...
    const missionFromAgentDoc = await safeReadTextFile((0, node_path_1.join)(AGENT_ROOT_DIR, "AGENTS.md"));
    ...
    try {
        ...
        for (const item of list) {
            ...
            members.push({
                agentId: id,
                displayName: (typeof obj.name === "string" && obj.name.trim()) || id,
...
function renderQuickFilters(filters, compactStatusStrip, section, language, usageView) {
    const options = ui_preferences_1.UI_QUICK_FILTERS.map((value) => ({
        value,
...
function humanizeOperatorLabel(value) {
...
function deriveAgentAnimalIdentity(agentId) {
    ...
    const matched = ANIMAL_CATALOG.find((item) => item.keywords.some((keyword) => compact.includes(keyword)));
    if (matched) {
        return {
            animal: matched.key,
            title: matched.title,
...
function buildOfficeSpaceCards(snapshot, tasks, knownAgentIds = [], runtimeActiveSessionsByAgent = new Map(), language = "zh") {
    ...
    for (const [agentId, activeCount] of runtimeActiveSessionsByAgent.entries()) {
        if (!agentId.trim() || activeCount <= 0)
            continue;
        if (!allowUnknownAgents && !configuredAgentKeys.has(normalizeLookupKey(agentId)))
            continue;
...
function buildExecutionAgentSummaries(snapshot, tasks, cronJobs, rosterEntries, usageAgentTokensByKey) {
    ...
    const registerAgent = (agentId, displayName) => {
        ...
        if (!allowUnknownAgents && !configuredRosterKeys.has(key))
            return;
        if (!canonicalIdByKey.has(key))
            canonicalIdByKey.set(key, normalized);
    ...
    for (const session of snapshot.sessions) {
        if (!session.agentId?.trim() || session.state === "idle")
            continue;
    ...
    return rows.sort((a, b) => {
        ...
        if (loadB !== loadA)
            return loadB - loadA;
        if (b.recentTokens30d !== a.recentTokens30d)
            return b.recentTokens30d - a.recentTokens30d;
        return a.agentId.localeCompare(b.agentId);
    });
}
...
function resolveOfficeCardStatus(states, activeSessionCount, activeTaskCount) {
    ...
    if (states.includes("blocked"))
        return "blocked";
    if (states.includes("waiting_approval"))
...
function officeStatusLabel(status, language = "zh") {
    ...
    if (status === "waiting_approval")
        return pickUiText(language, "Waiting for approval", "等待审批");
    if (status === "blocked")
        return pickUiText(language, "Needs support", "需要支援");
    if (status === "error")
        return pickUiText(language, "Issue detected", "发现异常");
    if (status === "idle")
        return pickUiText(language, "Standing by", "待命");
    if (status === "mixed")
        return pickUiText(language, "Mixed state", "混合状态");
    return pickUiText(language, "No active load", "无活跃负载");
}
function officeZoneFromStatus(status) {
    if (status === "running")
        return "Builder Desks";
    if (status === "waiting_approval")
        return "Approval Desk";
    if (status === "blocked" || status === "error" || status === "mixed")
        return "Support Bay";
    return "Standby Pods";
}
function officeZoneLabel(zone, language = "zh") {
    if (zone === "Builder Desks")
        return pickUiText(language, "Builder Desks", "执行工位");
    if (zone === "Approval Desk")
        return pickUiText(language, "Approval Desk", "审批工位");
    if (zone === "Support Bay")
        return pickUiText(language, "Support Bay", "支援工位");
    return pickUiText(language, "Standby Pods", "待命工位");
}
function animalLabel(animal, language = "zh") {
    if (animal === "robot")
        return pickUiText(language, "Robot", "机器人");
    if (animal === "lion")
        return pickUiText(language, "Lion", "狮子");
    if (animal === "panda")
        return pickUiText(language, "Panda", "熊猫");
    if (animal === "monkey")
        return pickUiText(language, "Monkey", "猴子");
    if (animal === "dolphin")
        return pickUiText(language, "Dolphin", "海豚");
    if (animal === "owl")
        return pickUiText(language, "Owl", "猫头鹰");
    if (animal === "fox")
        return pickUiText(language, "Fox", "狐狸");
    if (animal === "bear")
        return pickUiText(language, "Bear", "棕熊");
    if (animal === "eagle")
        return pickUiText(language, "Eagle", "鹰");
    if (animal === "tiger")
        return pickUiText(language, "Tiger", "老虎");
    if (animal === "otter")
        return pickUiText(language, "Otter", "水獭");
    if (animal === "rooster")
        return pickUiText(language, "Rooster", "公鸡");
...
function staffCurrentWorkLabel(input) {
    ...
    if (office?.status === "running") {
        if (focus)
            return { label: currentLabel, value: focus };
        if (cronName) {
    ...
    if (focus)
        return { label: nextLabel, value: focus };
    if (cronName) {
...
async function loadCachedStaffRecentActivity(snapshot, client, agentIds, language) {
    ...
    if (renderStaffRecentActivityCache &&
...
function pickRecentStaffActivity(history, language) {
    for (let idx = history.length - 1; idx >= 0; idx -= 1) {
        ...
        if (!isStaffVisibleOutputMessage(message))
            continue;
        return {
...
function historyImpliesStaffStopped(history) {
    for (let idx = history.length - 1; idx >= 0; idx -= 1) {
        const message = history[idx];
        if (isResidualPostStopMessage(message))
            continue;
        if (isExplicitStopSignalMessage(message))
            return true;
        return false;
    }
    return false;
...
function isExplicitStopSignalMessage(message) {
    ...
    return (lower.includes("停止当前任务并进入待命状态") ||
...
function isStaffVisibleOutputMessage(message) {
    ...
    if (!content)
        return false;
    const lower = content.toLowerCase();
    if (lower === "no_reply" || lower === "noop")
        return false;
    if (lower === "reply_skip" || lower === "announce_skip")
        return false;
    if (lower.startsWith("thinking "))
        return false;
    if (lower.startsWith("reasoning "))
        return false;
    if (lower.startsWith("text msg_"))
        return false;
    if (lower.startsWith("toolcall "))
        return false;
    if (lower.includes("agent-to-agent announce step"))
        return false;
    if (lower.includes("\"encrypted_content\""))
        return false;
    if (lower.includes("openclaw runtime context (internal)"))
        return false;
    if (lower.includes("conversation info (untrusted metadata)"))
        return false;
    if (lower.includes("internal_write_only"))
        return false;
    if (lower.includes("read heartbeat.md if it exists"))
        return false;
    if (content.includes("继续推进当前目标"))
        return false;
    if (content.includes("Alex Finn 流程最终验收"))
        return false;
    return true;
}
...
function renderStaffOverviewCards(cards, language = "zh") {
    if (cards.length === 0) {
...
function buildTaskExecutionChainCards(input) {
    ...
    const resolveCandidate = (sessionKey) => {
        ...
        if (!sessionLike)
            return undefined;
        const executionChain = preview?.executionChain ??
            (0, session_conversations_1.inferSessionExecutionChainFromSessionKey)({
    ...
    return `<div class="office-grid">${cards
        ...
        <div class="office-head">
          ...
          <div class="office-info">
            ...
            <div class="meta">${escapeHtml(pickUiText(language, "Active sessions", "活跃会话"))}：${card.activeSessions} · ${escapeHtml(pickUiText(language, "Active tasks", "活跃任务"))}：${card.activeTasks}</div>
          </div>
        </div>
...
function renderOfficeFloor(cards, language = "zh") {
    ...
    return `<div class="office-floor">${zones
        ...
        const rows = items.length === 0
            ...
            : items
                .map((card) => `<li class="desk-chip"><strong>${escapeHtml(humanizeOperatorLabel(card.agentId))}</strong><div class="meta">${escapeHtml(card.summary)}</div></li>`)
                .join("");
...
function renderParitySurfaceRows(rows) {
    return rows
        .map((row) => `<tr><td>${escapeHtml(row.name)}</td><td>${badge(row.status)}</td><td><a href="${escapeHtml(row.route)}">${escapeHtml(row.route)}</a></td><td>${escapeHtml(row.detail)}</td></tr>`)
        .join("");
}
...
function normalizeQuotaWindowLabel(label, fallback) {
    ...
    if (normalized === "7d" || normalized.includes("week") || normalized.includes("周"))
        return "Week";
    if (normalized === "5h" || normalized.includes("5h"))
        return "5h";
    ...
    if (minuteMatch?.[1]) {
        ...
        if (Number.isFinite(minutes)) {
            if (Math.abs(minutes - 10080) <= 2)
                return "Week";
            if (Math.abs(minutes - 300) <= 2)
                return "5h";
            if (Math.abs(minutes - 1440) <= 2)
                return "1d";
        }
    }
    return raw;
...
function renderQuotaWindowRow(input, language = "zh") {
    ...
    <div class="quota-head">
      ...
      <span class="quota-value">${escapeHtml(pickUiText(language, "Used", "已用"))} ${escapeHtml(usedText)} · ${escapeHtml(pickUiText(language, "Remaining", "剩余"))} ${escapeHtml(remainingText)}</span>
    </div>
...
function renderSubscriptionSidebarSummary(subscription, language = "zh") {
    if (subscription.status === "connected" && (subscription.primaryWindowLabel || subscription.secondaryUsedPercent !== undefined)) {
        ...
        const primaryRemaining = asPercent(subscription.primaryRemainingPercent ??
            (typeof primaryUsed === "number" ? 100 - primaryUsed : undefined));
        const secondaryUsed = asPercent(subscription.secondaryUsedPercent);
        const secondaryRemaining = asPercent(subscription.secondaryRemainingPercent ??
...
function renderSubscriptionStatusCard(subscription, language = "zh") {
    const statusLabel = subscription.status === "connected"
    ...
    if (subscription.status === "not_connected") {
        const hasEstimatedWindow = typeof subscription.consumed === "number" ||
            typeof subscription.remaining === "number" ||
    ...
    <div class="meta">${escapeHtml(pickUiText(language, "Used", "已用"))} ${escapeHtml(used)} · ${escapeHtml(pickUiText(language, "Remaining", "剩余"))} ${escapeHtml(remaining)}</div>
    <div class="meta">${escapeHtml(pickUiText(language, "Limit", "总额"))}：${escapeHtml(limit)} · ${escapeHtml(pickUiText(language, "Usage", "使用率"))}：${escapeHtml(usagePercent)}</div>
    <div class="meta">${escapeHtml(pickUiText(language, "Cycle", "周期"))}：${escapeHtml(cycleStart)} → ${escapeHtml(cycleEnd)}</div>
  </div>`;
}
function renderSubscriptionStatusCardForSmoke(subscription) {
    return renderSubscriptionStatusCard(subscription, "en");
}
function renderDashboardSectionNavForSmoke(section, language = "en") {
    const activeSection = section === "office-space"
        ? "team"
        : section === "calendar"
            ? "projects-tasks"
            : section;
...
function renderGlobalVisibilityCardForSmoke(language) {
    const model = {
        tasks: [
            {
                taskType: "cron",
...
function renderInformationCertaintyCardForSmoke(language) {
...
function renderTaskCertaintySectionForSmoke(language) {
...
function renderTaskExecutionChainCardsForSmoke(language = "zh") {
...
function renderUsagePeriodCards(periods, language = "zh") {
    ...
    const cards = periods
        ...
        const tokenLabel = period.sourceStatus === "not_connected"
            ? pickUiText(language, "Not connected", "数据源未连接")
            : formatInt(period.tokens);
        const costLabel = period.sourceStatus === "not_connected"
            ? pickUiText(language, "Not connected", "数据源未连接")
            : formatCurrency(period.estimatedCost);
        const requestLabel = period.requestCountStatus !== "not_connected" && typeof period.requestCount === "number"
...
function usagePeriodLabel(key, fallback, language = "zh") {
    if (key === "today")
        return pickUiText(language, "Today", "今天");
    if (key === "7d")
        return pickUiText(language, "Last 7 days", "近 7 天");
    if (key === "30d")
        return pickUiText(language, "Last 30 days", "近 30 天");
    return fallback;
}
...
function renderUsageBreakdownRows(rows, label, language = "zh") {
    if (rows.length === 0)
        return "";
    return rows
        .map((item) => `<tr><td>${escapeHtml(simplifyUsageLabel(item.label))}</td><td>${formatInt(item.tokens)}</td><td>${formatCurrency(item.estimatedCost)}</td><td>${item.requests}</td><td>${item.sessions}</td><td>${badge(item.sourceStatus, dataConnectionLabel(item.sourceStatus, language))}</td></tr>`)
        .join("");
}
...
function renderTokenPieChart(rows, totalTokens, centerLabel, language = "zh") {
    ...
    if (sourceRows.length === 0 || totalTokens <= 0)
        return "";
...
function renderUsageConnectorTodos(todos, language = "zh") {
    ...
    const simplifyTitle = (raw) => {
        ...
        if (lower.includes("context"))
            return pickUiText(language, "Model context data", "模型上下文数据");
        if (lower.includes("digest"))
            return pickUiText(language, "Trend history data", "趋势历史数据");
        if (lower.includes("request"))
            return pickUiText(language, "Request count data", "请求计数数据");
        if (lower.includes("budget"))
            return pickUiText(language, "Budget limit data", "预算限额数据");
        if (lower.includes("provider"))
            return pickUiText(language, "Provider mapping data", "供应商映射数据");
        if (lower.includes("subscription"))
            return pickUiText(language, "Subscription billing data", "订阅账单数据");
    ...
    const simplifyDetail = (raw) => {
        ...
        const lower = text.toLowerCase();
        if (lower.includes("subscription"))
            return pickUiText(language, "Connect the subscription billing data.", "请连接订阅账单数据。");
        if (lower.includes("digest") || lower.includes("history"))
            return pickUiText(language, "Keep monitoring running so trend history stays complete.", "请保持监控持续运行，确保趋势数据完整。");
        if (lower.includes("request"))
            return pickUiText(language, "Connect the request count source.", "请连接请求计数数据源。");
        if (lower.includes("provider"))
            return pickUiText(language, "Complete the model-to-provider mapping.", "请补全模型到供应商的映射。");
        if (lower.includes("context"))
            return pickUiText(language, "Connect model context capacity data.", "请连接模型上下文容量信息。");
        if (lower.includes("budget"))
            return pickUiText(language, "Configure budget limits so the system can warn early.", "请配置预算限额，便于风险预警。");
...
function dataConnectionLabel(status, language = "zh") {
    ...
    if (normalized === "connected")
        return pickUiText(language, "Connected", "已连接");
    if (normalized === "partial")
        return pickUiText(language, "Partially connected", "部分连接");
...
function isControlCenterMappingTask(task) {
    return (task.projectId === "p-live" &&
...
function renderTaskBoard(tasks, language = "zh") {
    if (tasks.length === 0) {
    ...
    const lanes = TASK_STATES.map((state) => {
        ...
        const cards = laneTasks.length === 0
            ...
            : laneTasks
                .map((task) => `<div class="task-chip"><div><code>${escapeHtml(task.taskId)}</code></div><div>${escapeHtml(task.title)}</div><div class="meta">${escapeHtml(task.projectId)} · ${escapeHtml(task.owner)}</div></div>`)
                .join("");
...
function renderSessionDrilldownPageForSmoke(detail, language = "en") {
...
function renderAuditPage(timeline, severity) {
    const rows = timeline.events.length === 0
        ? "<tr><td colspan=\"4\">Not activated yet</td></tr>"
        : timeline.events
            .slice(0, 300)
            .map((event) => `<tr><td>${escapeHtml(event.timestamp)}</td><td>${badge(event.severity)}</td><td>${escapeHtml(event.source)}</td><td>${escapeHtml(event.message)}</td></tr>`)
            .join("");
    return `<!doctype html>
<html>
<head>
...
function renderAuditPageForSmoke(timeline, severity) {
...
function renderExceptionsList(feed) {
    ...
    if (top.length === 0)
        return "<li>暂无异常</li>";
    return top
        .map((item) => `<li>${badge(item.level)} <strong>${escapeHtml(item.message)}</strong> <span class="meta-inline">${escapeHtml(item.sourceId)} · ${escapeHtml(item.occurredAt ?? "-")}</span></li>`)
        .join("");
}
...
function resolveDocPath(docId) {
    ...
    if (normalized === "runbook")
        return (0, node_path_1.join)(DOCS_DIR, "RUNBOOK.md");
    if (normalized === "architecture")
        return (0, node_path_1.join)(DOCS_DIR, "ARCHITECTURE.md");
    if (normalized === "progress")
        return (0, node_path_1.join)(DOCS_DIR, "PROGRESS.md");
    return undefined;
}
...
function formatSeconds(value, language = "en") {
    ...
    if (language === "zh") {
        ...
        const rem = abs % 60;
        if (minutes < 60)
            return rem === 0 ? `${sign}${minutes}分` : `${sign}${minutes}分${rem}秒`;
...
function formatMs(value, language = "en") {
    if (!Number.isFinite(value))
        return "-";
    return formatSeconds(Math.round(value / 1000), language);
}
...
function writeJson(res, statusCode, body) {
    writeText(res, statusCode, JSON.stringify(attachRequestIdToBody(body, responseRequestId(res)), null, 2), "application/json; charset=utf-8");
}
...
function assertMutationAuthorized(req, routeLabel, explicitToken) {
    const token = (0, local_token_auth_1.normalizeToken)(explicitToken) ??
        (0, local_token_auth_1.normalizeToken)(readHeaderValue(req, config_1.LOCAL_TOKEN_HEADER)) ??
        (0, local_token_auth_1.normalizeToken)((0, local_token_auth_1.readAuthorizationBearer)(readHeaderValue(req, "authorization")));
    const decision = (0, local_token_auth_1.evaluateLocalTokenGate)({
        gateRequired: config_1.LOCAL_TOKEN_AUTH_REQUIRED,
        configuredToken: config_1.LOCAL_API_TOKEN,
        providedToken: token,
        routeLabel,
    });
    if (!decision.ok) {
...
function assertAllowedQueryParams(searchParams, allowed, strict) {
    ...
    if (unknown.length === 0)
        return;
    throw new RequestValidationError(`Unknown query parameter(s): ${unknown.join(", ")}`, 400, unknown.map((key) => `query '${key}' is not supported`));
}
...
function readPositiveIntQuery(value, label, fallback, strict, maxValue = 1000) {
    if (!value)
        return fallback;
    const trimmed = value.trim();
    if (!trimmed)
    ...
    if (!Number.isFinite(parsed) || parsed <= 0 || parsed > maxValue) {
        if (strict)
            throw new RequestValidationError(`${label} must be an integer in range 1..${maxValue}`, 400);
        return fallback;
    }
    return parsed;
}
