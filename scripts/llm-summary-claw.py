...
# -*- coding: utf-8 -*-
"""
使用 OpenClaw 内置的 GLM-4.5-Air 进行记忆摘要
"""
...
# ============================================================================
# LLM 接口（使用 OpenClaw 内置的 GLM-4.5-Air）
# ============================================================================

def call_llm_claw(prompt: str, max_tokens: int = 1000) -> str:
    """
    使用 OpenClaw 内置的 GLM-4.5-Air
    """
    try:
        # 使用 openclaw CLI 调用 GLM
        log(f"调用 OpenClaw GLM-4.5-Air（prompt: {len(prompt)} 字符）", "DEBUG")
...
请用简洁、准确的方式完成任务。只返回结果，不要有多余的废话。"""
        ...
        result = subprocess.run(
            ["openclaw", "chat", "--model", "glmcode/glm-4.5-air", "--json"],
        ...
        if result.returncode == 0:
            ...
            log(f"GLM-4.5-Air 返回: {len(content)} 字符", "DEBUG")
            return content
        else:
            log(f"OpenClaw CLI 失败: {result.stderr}", "ERROR")
            return _rule_based_extraction(prompt)
        
    except Exception as e:
        log(f"OpenClaw 调用失败: {e}，使用规则提取", "WARNING")
        return _rule_based_extraction(prompt)
...
- 关键点2
..."""
        key_points = call_llm_claw(prompt, max_tokens=800)
...
def process_l0_to_l1(target_date: Optional[str] = None):
    """处理 L0 → L1"""
    log("开始处理 L0 → L1: 每日对话 → 关键点（使用 OpenClaw GLM-4.5-Air）", "INFO")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="使用 OpenClaw GLM-4.5-Air 的记忆摘要生成")
    parser.add_argument("--date", help="指定日期 (YYYY-MM-DD)")
    parser.add_argument("--test", action="store_true", help="测试 LLM 连接")

    args = parser.parse_args()

    if args.test:
        # 测试 LLM
        log("测试 OpenClaw GLM-4.5-Air 连接...", "INFO")
        test_prompt = "请简单介绍一下你自己，不超过50字"
        result = call_llm_claw(test_prompt, max_tokens=200)
